...
static void mpxplay_main_cycle(void)
{
 ...
 switch(mvp->partselect){
  case 1:funcbit_smp_value_put(mtc1_running,1);main_part1(mvp);funcbit_smp_value_put(mtc1_running,0);break;
...
static void crossfade_manual_set(struct mainvars *mvp,unsigned int newpart)
{
 ...
 switch(newpart){
  case CROSS_OUT  :mvp->cfi->crosswait=0;
                   if(mvp->newfilenum)
		    mvp->step=0;
		   else{
  ...
  case CROSS_IN   :mpxplay_infile_reset(mvp->frp0+1);
