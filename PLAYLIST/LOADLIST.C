  ...
  if(data && data[1]){
   ...
   switch(type){
    case PDS_GET4C_LE32('F','I','D','A'):{
     ...
     break;}
    case PDS_GET4C_LE32('F','I','S','I'):plx->filesize=pds_atol(data);break;
...
unsigned int playlist_loadlist_get_header_by_ext(struct playlist_side_info *psi,struct playlist_entry_info *pei,char *filename)
{
 ...
 if(!pei->id3info[I3I_ARTIST] && !pei->id3info[I3I_TITLE])
  switch(pei->entrytype){
   case DFT_PLAYLIST:pei->id3info[I3I_DFT_STORE]=DFTSTR_PLAYLIST;break;
   case DFT_SUBLIST :pei->id3info[I3I_DFT_STORE]=DFTSTR_SUBLIST;break;
  }
 return 1;
}
