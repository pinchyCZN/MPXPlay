...
void playlist_fileinfo_show_infos(struct mainvars *mvp)
{
	...
	if(frp->infile_funcs) {
		...
		if(adi->channeltext) {
			...
			case 1:
				len += pds_strcat(msg, "Mono\n");
				break;
			case 2:
				len += pds_strcat(msg, "Stereo\n");
				break;
			default:
				sprintf(strtmp, "%d\n", adi->filechannels);
				len += pds_strcat(msg, strtmp);
	...
	if(ff_result == 0) {
		...
		sprintf(strtmp, "\nFiledate: %4.4d.%2.2d.%2.2d. %2d:%2.2d", fdate->year + 1980, fdate->month, fdate->day, fdate->hours, fdate->minutes);
		len += pds_strcat(msg, strtmp);
		if(is_lfn_support && (uselfn & USELFN_ENABLED)) {
			...
			if(!cdate->year && !cdate->month && !cdate->day && !cdate->hours && !cdate->minutes)
				...
				sprintf(strtmp, "\nCreated : %4.4d.%2.2d.%2.2d. %2d:%2.2d", cdate->year + 1980, cdate->month, cdate->day, cdate->hours, cdate->minutes);
			len += pds_strcat(msg, strtmp);
			if(!adate->year && !adate->month && !adate->day && !adate->hours && !adate->minutes)
				sprintf(strtmp, "\nAccessed: -");
			else
				sprintf(strtmp, "\nAccessed: %4.4d.%2.2d.%2.2d. %2d:%2.2d", adate->year + 1980, adate->month, adate->day, adate->hours, adate->minutes);
			len += pds_strcat(msg, strtmp);
		}
...
static void editfileinfos_tagwrite_errorhandler(struct editfileinfo_s *efi, unsigned int extkey)
{
	...
	case 0x1e61:
	case 0x1e41:
		funcbit_enable(efi->flags, EDITFILEINFOS_FLAG_IGNORE_ERRORS);
		mpxplay_timer_addfunc(editfileinfos_update_all_loop, efi, MPXPLAY_TIMERTYPE_REPEAT | MPXPLAY_TIMERFLAG_LOWPRIOR, 0);
		break;
	default:
		mpxplay_timer_addfunc(editfileinfos_dealloc, efi, MPXPLAY_TIMERFLAG_INDOS, 0);
	}
}
...
static void editfileinfos_get_errormsg(struct editfileinfo_s *efi, int error, char *sbuf, int sbufsize)
{
	...
	switch (error) {
	case MPXPLAY_ERROR_INFILE_OK:	/*pds_strcpy(sout,"Tags saved"); */
		*sbuf = 0;
		return;
	case MPXPLAY_ERROR_INFILE_CANTOPEN:
		snprintf(sbuf, sbufsize, "Write error (read-only or not exists) at\n%s", shortfname);
		break;
	case MPXPLAY_ERROR_INFILE_WRITETAG_FILETYPE:
		snprintf(sbuf, sbufsize, "Tag-write is not supported for this filetype\n%s", shortfname);
		break;
	case MPXPLAY_ERROR_INFILE_WRITETAG_TAGTYPE:
		snprintf(sbuf, sbufsize, "Wrong or unsupported tagtype in\n%s", shortfname);
		break;
	case MPXPLAY_ERROR_FILEHAND_CANTWRITE:
		snprintf(sbuf, sbufsize, "Cannot write to file\n%s", shortfname);
		break;
		//case MPXPLAY_ERROR_FILEHAND_CHANGEATTR:snprintf(sbuf,sbufsize,"Cannot change the attrib of\n%s",shortfname);break;
	case MPXPLAY_ERROR_INFILE_WRITETAG_IV2_SPACE:
		snprintf(sbuf, sbufsize, "Not enough space to store all frames in ID3V2!\nMaybe some tag-infos have lost in\n%s", shortfname);
		return;
	default:
		snprintf(sbuf, sbufsize, "Tag-write (%d) error at\n%s", error, shortfname);
		break;
	}
...
void playlist_fileinfo_edit_infos(struct mainvars *mvp)
{
	...
	if(psi->selected_files) {
		...
		if(pei->infobits & PEIF_INDEXED) {
			...
			display_textwin_additem_msg_static(tw, 0, x, y, "Start time:");
			...
			display_textwin_additem_editline(tw, 0, eflags, x, y, 3, efi->pstimestr_edited, 3);
			x += 3;
			display_textwin_additem_msg_static(tw, 0, x, y, &efi->pstimestr_edited[3]);
			x++;
			display_textwin_additem_editline(tw, 0, eflags, x, y, 2, &efi->pstimestr_edited[4], 2);
			x += 2;
			display_textwin_additem_msg_static(tw, 0, x, y, &efi->pstimestr_edited[6]);
			x++;
			display_textwin_additem_editline(tw, 0, eflags, x, y, 2, &efi->pstimestr_edited[7], 2);
			x += 2 + 2;

			display_textwin_additem_msg_static(tw, 0, x, y, "Length:");
			x += sizeof("Length:") - 1;
			editfileinfos_maketimemstr(efi->pltimestr_origi, lentime);
			pds_strcpy(efi->pltimestr_edited, efi->pltimestr_origi);
			display_textwin_additem_editline(tw, 0, eflags, x, y, 3, efi->pltimestr_edited, 3);
			x += 3;
			display_textwin_additem_msg_static(tw, 0, x, y, &efi->pltimestr_edited[3]);
			x++;
			display_textwin_additem_editline(tw, 0, eflags, x, y, 2, &efi->pltimestr_edited[4], 2);
			x += 2;
			display_textwin_additem_msg_static(tw, 0, x, y, &efi->pltimestr_edited[6]);
			x++;
			display_textwin_additem_editline(tw, 0, eflags, x, y, 2, &efi->pltimestr_edited[7], 2);
			x += 2 + 2;

			display_textwin_additem_msg_static(tw, 0, x, y, "End time:");
			x += sizeof("End time:") - 1;
			editfileinfos_maketimemstr(efi->petimestr_origi, endtime);
			pds_strcpy(efi->petimestr_edited, efi->petimestr_origi);
			display_textwin_additem_editline(tw, 0, eflags, x, y, 3, efi->petimestr_edited, 3);
			x += 3;
			display_textwin_additem_msg_static(tw, 0, x, y, &efi->petimestr_edited[3]);
			x++;
			display_textwin_additem_editline(tw, 0, eflags, x, y, 2, &efi->petimestr_edited[4], 2);
			x += 2;
			display_textwin_additem_msg_static(tw, 0, x, y, &efi->petimestr_edited[6]);
			x++;
			display_textwin_additem_editline(tw, 0, eflags, x, y, 2, &efi->petimestr_edited[7], 2);
			x += 2 + 2;
