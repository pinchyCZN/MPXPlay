		...
		for(pei = psi->firstsong; pei <= psi->lastentry; pei++) {
			if((list_type & PLST_EXTM3U) && (pei->infobits & PEIF_ENABLED)) {
				snprintf(sout, sizeof(sout), "#EXTINF:%d,%.200s - %.200s",
						 (pei->timemsec + 500) / 1000, mpxplay_playlist_textconv_back(cnvtmp1, pei->id3info[I3I_ARTIST]), mpxplay_playlist_textconv_back(cnvtmp2, pei->id3info[I3I_TITLE]));
				if(mpxplay_diskdrive_textfile_writeline(fp, sout) <= 0)
					return MPXP_SAVELIST_RETCODE_CANTWRITE;
			}
...
static unsigned int save_mxu_playlist(struct playlist_side_info *psi, void *fp)
{
	...
	for(pei = psi->firstsong; pei <= psi->lastentry; pei++) {
		snprintf(sout, sizeof(sout), "%.400s°%.200s±%.200s²%8.8X", (pei->filename) ? pei->filename : "",
				 ...
				 mpxplay_playlist_textconv_back(cnvtmp2, pei->id3info[I3I_TITLE]), (pei->infobits & PEIF_ENABLED) ? (MXUFLAG_ENABLED | (((pei->timemsec + 500) / 1000) & MXUFLAG_TIMEMASK)) : 0);
		if(mpxplay_diskdrive_textfile_writeline(fp, sout) <= 0)
			return MPXP_SAVELIST_RETCODE_CANTWRITE;
	}
	return MPXP_SAVELIST_RETCODE_OK;
...
static unsigned int savelist_open_and_write_list(struct playlist_side_info *psi, char *filename, unsigned int list_type, unsigned int cuefullinfo)
{
	...
	switch (list_type & PLST_LISTS) {
	case PLST_MXU:
		retcode = save_mxu_playlist(psi, fp);
		break;
	case PLST_CUE:
		retcode = save_cue_playlist(psi, fp, path, cuefullinfo);
		break;
	default:
		retcode = save_m3u_playlist(psi, fp, path, list_type);
		break;
	}
...
static void manual_savelist_keyhand(struct playlist_side_info *psi, unsigned int extkey)
{
	...
	case 0x1265:
	case 0x1245:
		psi->savelist_type = PLST_EXTM3U;
		break;
	case 0x326d:
	case 0x324d:
		psi->savelist_type = PLST_M3U;
		break;
	case 0x2e63:
	case 0x2e43:
		psi->savelist_type = PLST_CUE;
		break;
	case 0x2d78:
	case 0x2d58:
		psi->savelist_type = PLST_MXU;
		break;
	default:
		return;
	}
...
void playlist_savelist_manual_save(struct mainvars *mvp)
{
	...
	case PLST_EXTM3U:
		btsel = &savelist_buttons[1];
		break;
	case PLST_CUE:
		btsel = &savelist_buttons[5];
		break;
	case PLST_MXU:
		btsel = &savelist_buttons[7];
		funcbit_disable(texttype, MPXPLAY_DISKDRIVEDATA_UTFTYPE_UTF_ALL);
		break;
	default:
		btsel = &savelist_buttons[3];
	}
	...
	case MPXPLAY_DISKDRIVEDATA_UTFTYPE_UTF8:
		pds_strcpy(msg, "UTF-8");
		break;
	case MPXPLAY_DISKDRIVEDATA_UTFTYPE_UTF16LE:
		pds_strcpy(msg, "UTF-16LE");
		break;
	case MPXPLAY_DISKDRIVEDATA_UTFTYPE_UTF16BE:
		pds_strcpy(msg, "UTF-16BE");
		break;
	default:
		pds_strcpy(msg, "UTF-8");	// !!! default forced type (if user enables the switch)
