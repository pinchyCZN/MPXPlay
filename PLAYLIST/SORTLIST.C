...
static struct playlist_entry_info *sortlist_order_one_block(struct playlist_side_info *psi,
															struct playlist_entry_info *pei_src, struct playlist_entry_info *firstentry, struct playlist_entry_info *lastentry)
{
...
{
	switch (select) {
	case PLAYLIST_SORTLIST_SELECTFUNCS_PLAYER:
		pds_memcpy(check_order_funcs, check_order_func_player, sizeof(check_order_funcs));
		break;
	case PLAYLIST_SORTLIST_SELECTFUNCS_COMMANDER:
		pds_memcpy(check_order_funcs, check_order_func_commander, sizeof(check_order_funcs));
		break;
	}
}
...
void playlist_order_block(struct playlist_side_info *psi, struct playlist_entry_info *firstentry, struct playlist_entry_info *lastentry)
{
	struct mainvars *mvp;
...
static void playlist_order_side_block(struct playlist_side_info *psi, struct playlist_entry_info *firstentry, struct playlist_entry_info *lastentry)
{
	if(lastentry > firstentry) {
...
static void playlist_order_dft_block(struct playlist_side_info *psi, struct playlist_entry_info *firstentry, struct playlist_entry_info *lastentry)
{
	check_order_func_t *ot_save[PLAYLIST_MAX_ORDERKEYS];
...
void playlist_order_filenames_block(struct playlist_side_info *psi, struct playlist_entry_info *firstentry, struct playlist_entry_info *lastentry)
{
	if(!psi->id3ordertype[0])
...
void playlist_swap_entries(struct playlist_side_info *psi, struct playlist_entry_info *e1, struct playlist_entry_info *e2)
{
	...
	if(psi == mvp->psip) {
		if(mvp->aktfilenum == e1)
			mvp->aktfilenum = e2;
		else if(mvp->aktfilenum == e2)
			mvp->aktfilenum = e1;
		if(mvp->newfilenum == e1)
			mvp->newfilenum = e2;
		else if(mvp->newfilenum == e2)
			mvp->newfilenum = e1;
