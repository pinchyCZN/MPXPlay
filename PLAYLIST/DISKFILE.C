...
static void diskfile_count_files_updatemsg(struct filecopy_t *fc)
{
	...
	if(fc->cpy_type & (DISKFILE_CPYTYPE_INFO_SIZE | DISKFILE_CPYTYPE_INFO_TIME))
		...
		if(fc->cpy_type & DISKFILE_CPYTYPE_DEL)
			sprintf(sout, "(total %d file%s and %d subdir%s)", fc->counted_filenum_all, ((fc->counted_filenum_all > 1) ? "s" : ""), fc->counted_directories,
					((fc->counted_directories > 1) ? "s" : ""));
		else {
			if(fc->psi_src->selected_files) {
				sprintf(sout, " %s %d file%s (total %d file%s with %d subdir%s) to",
						...
						fc->counted_filenum_all, ((fc->counted_filenum_all > 1) ? "s" : ""), fc->counted_directories, ((fc->counted_directories > 1) ? "s" : ""));
			} else {
...
static void diskfile_count_files_in_subdirs(struct filecopy_t *fc)
{
	...
	if((pei->entrytype == DFT_DRIVE) || (pei->entrytype == DFT_UPDIR) || (pei->entrytype == DFT_SUBDIR)) {
		...
		if(!(fc->count_ctrl & DISKFILE_COUNTCTRL_NEXTFILE)) {
			...
			case DFT_DRIVE:
				len = pds_strncpy(searchpath, pei->filename, sizeof(PDS_DIRECTORY_DRIVE_STR) - 1);
				break;
			case DFT_UPDIR:
				len = pds_strcpy(searchpath, fc->psi_src->currdir);
				break;
			default:
				len = pds_strcpy(searchpath, pei->filename);
			}
...
static void diskfile_filecopy_errorhandler(struct filecopy_t *fc, unsigned int extkey)
{
	...
	case 0x1e61:
	case 0x1e41:
		funcbit_enable(fc->cpy_ctrl, DISKFILE_CPYCTRL_IGNALLERR);
	case 0x1769:
	case 0x1749:
...
static void playlist_diskfile_show_errmsg(struct filecopy_t *fc)
{
	...
	case MPXPLAY_ERROR_FILEHAND_OK:
		break;
	case MPXPLAY_ERROR_FILEHAND_USERABORT:	/*diskfile_filecopy_errormessage(fc,"Copy aborted by user!"); */
		break;
	case MPXPLAY_ERROR_FILEHAND_MEMORY:
		diskfile_filecopy_errormessage(fc, "Memory allocation error at", fc->infilename);
		break;
	case MPXPLAY_ERROR_FILEHAND_CANTOPEN:
		diskfile_filecopy_errormessage(fc, "Cannot open for reading at", fc->infilename);
		break;
	case MPXPLAY_ERROR_FILEHAND_CANTCREATE:
		diskfile_filecopy_errormessage(fc, "Cannot create", fc->lastfilename);
		break;
	case MPXPLAY_ERROR_FILEHAND_CANTREAD:
		diskfile_filecopy_errormessage(fc, "Cannot read file (Where is the disk?)", fc->infilename);
		break;
	case MPXPLAY_ERROR_FILEHAND_CANTWRITE:
		diskfile_filecopy_errormessage(fc, "Cannot write file (Disk full?)", fc->lastfilename);
		break;
	case MPXPLAY_ERROR_FILEHAND_DELETE:
		diskfile_filecopy_errormessage(fc, "Couldn't delete file", fc->infilename);
		break;
	case MPXPLAY_ERROR_FILEHAND_RENAME:
		diskfile_filecopy_errormessage(fc, "Couldn't rename/move file", fc->infilename);
		break;
	case MPXPLAY_ERROR_FILEHAND_REMOVEDIR:
		diskfile_filecopy_errormessage(fc, "Couldn't delete directory", fc->lastfilename);
		break;
	case MPXPLAY_ERROR_FILEHAND_CHANGEATTR:
		diskfile_filecopy_errormessage(fc, "Couldn't modify the attribs of", fc->infilename);
		break;
	case MPXPLAY_ERROR_FILEHAND_CANTPERFORM:
		diskfile_filecopy_errormessage(fc, "Cannot perform operation on this filetype", fc->infilename);
		break;
	case MPXPLAY_ERROR_FILEHAND_CANTCOPY:
		diskfile_filecopy_errormessage(fc, "Cannot copy/move (probably a correct fullpath is missing)!", fc->lastfilename);
		break;
	case MPXPLAY_ERROR_FILEHAND_COPYDIR:
		diskfile_filecopy_errormessage(fc, "Cannot copy/move/del drive/updir/uplist", fc->infilename);
		break;
	case MPXPLAY_ERROR_FILEHAND_SAMEDIR:
		diskfile_filecopy_errormessage(fc, "Cannot copy/move a directory to itself!", fc->lastfilename);
		break;
	case MPXPLAY_ERROR_FILEHAND_MULTITO1:
		diskfile_filecopy_errormessage(fc, "Cannot copy/move multiply files to one!\nDestination must be an existent path:", fc->outpath_argument);
		break;
	case MPXPLAY_ERROR_FILEHAND_SKIPFILE:
		break;
	default:
		diskfile_filecopy_errormessage(fc, "Unknown error in copy at", fc->infilename);
		break;
	}
}
...
static void diskfile_filecopymove_existfile_keyhand(struct filecopy_t *fc, unsigned int extkey)
{
	...
	case 0x184f:
		if(pds_stricmp(fc->outfilename, fc->lastfilename) == 0) {	// if the filename is not modified manually
	...
	case 0x324d:
		if(diskfile_filecopymove_make_subdir(fc, fc->outfilename) == MPXPLAY_ERROR_FILEHAND_OK)
			mpxplay_timer_addfunc(diskfile_filecopymove_makefilenames, fc, MPXPLAY_TIMERFLAG_INDOS, 0);
		else
	...
	case 0x1e41:
		if(pds_stricmp(fc->outfilename, fc->lastfilename) == 0) {
	...
	case 0x1f73:				// skip
	case 0x1f53:
		funcbit_enable(fc->cpy_ctrl, DISKFILE_CPYCTRL_SKIPFILE);
		fc->retcode = MPXPLAY_ERROR_FILEHAND_SKIPFILE;
		break;
	case 0x256b:				// skip all
	case 0x254b:
		funcbit_enable(fc->cpy_ctrl, (DISKFILE_CPYCTRL_SKIPFILE | DISKFILE_CPYCTRL_ALLFILE));
		fc->retcode = MPXPLAY_ERROR_FILEHAND_SKIPFILE;
		break;
	case 0x2e63:				// cancel
	case 0x2e43:
...
static void diskfile_filemovedel_delsubdir_keyhand(struct filecopy_t *fc, unsigned int extkey)
{
	...
	case 0x2064:
	case 0x2044:
		funcbit_enable(fc->cpy_ctrl, DISKFILE_CPYCTRL_DELSUBDIR);
		break;
	case 0x1e61:
	case 0x1e41:
		funcbit_enable(fc->cpy_ctrl, (DISKFILE_CPYCTRL_DELSUBDIR | DISKFILE_CPYCTRL_ALLSUBDIR));
		break;
	case 0x1f73:
	case 0x1f53:
		fc->pei_selected++;
...
static int diskfile_filecopymove_getnextfile(struct filecopy_t *fc)
{
	...
	switch (pei->entrytype) {
	case DFT_UPDIR:
		pds_strcpy(srcname, fc->psi_src->currdir);
		break;
	default:
		pds_strcpy(srcname, pei->filename);
	}
...
static void diskfile_filecopymove_checkfilenames(struct filecopy_t *fc)
{
  ...
  err_out_noext:
	...
	diskfile_statwin_close(fc);
	tw = display_textwin_allocwindow_items(NULL, TEXTWIN_FLAG_ERRORMSG | TEXTWIN_FLAG_MSGCENTERALIGN, fc->headtext, diskfile_filecopymove_existfile_keyhand, fc);
	display_textwin_additem_msg_static(tw, TEXTWIN_FLAG_MSGCENTERALIGN, 0, -1,
									   "You perform a file copy/move without extension\nor the target directory doesn't exists.\nYou can continue or give an other target:");
	display_textwin_additem_editline(tw, TEXTWIN_FLAG_MSGCENTERALIGN, 0, 0, -1, 50, fc->outfilename, MAX_PATHNAMELEN - 2);
...
static void diskfile_filecopymove_postprocess(struct filecopy_t *fc)
{
	...
	if(fc->pei_curr && (fc->cpy_type & (DISKFILE_CPYTYPE_COPY | DISKFILE_CPYTYPE_MOVE | DISKFILE_CPYTYPE_RENAME))) {
		...
		if((fc->retcode == MPXPLAY_ERROR_FILEHAND_OK) || (fc->retcode == MPXPLAY_ERROR_FILEHAND_CANTREAD) || (fc->retcode == MPXPLAY_ERROR_FILEHAND_CANTWRITE)
		   || (fc->retcode == MPXPLAY_ERROR_FILEHAND_USERABORT) || (fc->retcode == MPXPLAY_ERROR_FILEHAND_DELETE)) {
			pds_getpath_from_fullname(fc->outpath_curr, fc->outfilename);
	...
	if(fc->pei_curr) {			// if not a subdir-filename
		...
		if(((fc->cpy_type & (DISKFILE_CPYTYPE_COPY | DISKFILE_CPYTYPE_RENAME)) && (fc->retcode == MPXPLAY_ERROR_FILEHAND_OK)) || (fc->retcode == MPXPLAY_ERROR_FILEHAND_DELETE)
		   || (fc->retcode == MPXPLAY_ERROR_FILEHAND_SKIPFILE)) {
...
static void diskfile_copymove_start(struct filecopy_t *fc, unsigned int extkey)
{
	...
	case 0x2166:				// 'f'
	case 0x2146:
		funcbit_disable(fc->cpy_ctrl, DISKFILE_CPYCTRL_WITHSUBDIR);
	case 0x1f73:				// 's'
	case 0x1f53:				// 'S'
		diskfile_count_files_stop(fc);
...
static void diskfile_filedel_delcurrplay_keyhand(struct filecopy_t *fc, unsigned int extkey)
{
	...
	case 0x2064:
	case 0x2044:
		funcbit_enable(fc->cpy_ctrl, DISKFILE_CPYCTRL_DELFILE);
		playlist_diskfile_delete_do(fc);
		break;
	case 0x1f73:
	case 0x1f53:
...
static void diskfile_multifileinfos_buttonhandler(struct filecopy_t *fc, unsigned int extkey)
{
	...
	case 0x3d00:
	case KEY_ESC:
		diskfile_multifileinfos_close(fc);
		return;
	}
}
...
static void diskfile_show_multifileinfos_window(struct filecopy_t *fc)
{
	...
	snprintf(sout, sizeof(sout),
			 ...
			 fc->counted_directories, fc->counted_filenum_media, (float)fc->counted_filesizes_media / 1048576.0, (long)((fc->counted_timemsec_media + 500) / 3600000),
			 ((long)((fc->counted_timemsec_media + 500) / 60000)) % 60, ((long)((fc->counted_timemsec_media + 500) / 1000)) % 60, fc->counted_filenum_list,
			 (float)fc->counted_filesizes_list / 1048576.0, fc->counted_filenum_other, ((float)(fc->counted_filesizes_all - fc->counted_filesizes_media - fc->counted_filesizes_list) / 1048576.0),
...
static unsigned int diskfile_renamebyid3_createoutfilename(struct filecopy_t *fc, struct playlist_entry_info *pei, char *outbuf, unsigned int bufsize)
{
	...
	if(tracknum) {				// !!!
		...
		case 1:				//
		case 2:
			snprintf(outbuf, bufsize, "%2.2d. %.60s%s%.60s%s", tracknum, &artist[0], ((artist[0] && title[0]) ? " - " : ""), &title[0], ((ext) ? ext : ""));
			break;
		case 3:
			snprintf(outbuf, bufsize, "%3.3d. %.60s%s%.60s%s", tracknum, &artist[0], ((artist[0] && title[0]) ? " - " : ""), &title[0], ((ext) ? ext : ""));
			break;
		case 4:
			snprintf(outbuf, bufsize, "%4.4d. %.60s%s%.60s%s", tracknum, &artist[0], ((artist[0] && title[0]) ? " - " : ""), &title[0], ((ext) ? ext : ""));
			break;
		case 5:
			snprintf(outbuf, bufsize, "%5.5d. %.60s%s%.60s%s", tracknum, &artist[0], ((artist[0] && title[0]) ? " - " : ""), &title[0], ((ext) ? ext : ""));
			break;
		default:
			snprintf(outbuf, bufsize, "%6.6d. %.60s%s%.60s%s", tracknum, &artist[0], ((artist[0] && title[0]) ? " - " : ""), &title[0], ((ext) ? ext : ""));
			break;
		}
	} else {
		snprintf(outbuf, bufsize, "%.60s%s%.60s%s", &artist[0], ((artist[0] && title[0]) ? " - " : ""), &title[0], ((ext) ? ext : ""));
	}
...
void playlist_diskfile_rename_by_id3(struct mainvars *mvp)
{
	...
	if(psi->selected_files)
		sprintf(msg, " Rename the selected %d file%s \n to \"NN. Artist - Title\" format by ID3-info %s", psi->selected_files, ((psi->selected_files > 1) ? "s" : ""),
				((fc->path_src[0]) ? "in " : ""));
	else
