...
#endif
   switch(di->type){
    case DRIVE_TYPE_FLOPPY :pei->id3info[I3I_DFT_STORE]=((i<2)? "<FLOPPY>":"<REMOVABLE>");break;
    case DRIVE_TYPE_CD     :pei->id3info[I3I_DFT_STORE]="<CD_ROM>";break;
    case DRIVE_TYPE_NETWORK:pei->id3info[I3I_DFT_STORE]="<NETWRK>";break;
    case DRIVE_TYPE_RAMDISK:pei->id3info[I3I_DFT_STORE]="<RAMDISK>";break;
    default:pei->id3info[I3I_DFT_STORE]=DFTSTR_DRIVE;break;
   }
   *fnp++=0;
   pei->mdds=&di->mdds;
   pei++;
...
static struct playlist_entry_info *browser_get_files_from_dir(struct playlist_entry_info *pei,
              struct playlist_side_info *psi,char **filenamesfieldp)
{
 unsigned int error,len;
 struct mpxplay_diskdrive_data_s *mdds=psi->mdds;
 struct pds_find_t ffblk;
