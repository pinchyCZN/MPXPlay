...
char *get_playstatus_string(unsigned int *color)
{
	...
	if(!(playcontrol & PLAYC_RUNNING))
		...
		else if(playcontrol & PLAYC_HIGHSCAN)
			status = 6;
		else
			status = 4;
	}
...
static void display_fileinfos(struct mainvars *mvp)
{
	...
	if(displaymode & DISP_VERBOSE) {
		...
		if(id3ip[I3I_ARTIST] || id3ip[I3I_TITLE]) {
			snprintf(souttext, sizeof(souttext), "Mpxplay v1.57 - %s : %s ", (id3ip[I3I_ARTIST]) ? id3ip[I3I_ARTIST] : "", (id3ip[I3I_TITLE]) ? id3ip[I3I_TITLE] : "");
		} else
		...
		if(adi->channeltext)
			...
			case 2:
				...
				break;
			default:
				snprintf(channumstr, sizeof(channumstr), "%2d-chan ", adi->filechannels);
				channeltext = &channumstr[0];
				break;
			}
		}
		if(displaymode & DISP_NOFULLEDIT) {
			...
			if(adi->bitratetext) {
				snprintf(souttext, sizeof(souttext), "%s, %s, %ld Hz, %s            ", filetypestr, adi->bitratetext, adi->freq, channeltext);
			} else {
				if(adi->bitrate) {
					snprintf(souttext, sizeof(souttext), "%s, %d kbit/s, %ld Hz, %s            ", filetypestr, adi->bitrate, adi->freq, channeltext);
				} else {
					snprintf(souttext, sizeof(souttext), "%s, %d bit, %ld Hz, %s            ", filetypestr, adi->bits, adi->freq, channeltext);
				}
			}
...
static void editor_draw_one_id3info_editor_line(struct playlist_entry_info *pei, unsigned int lastid,
												unsigned int col, unsigned int endcol, unsigned int color, unsigned int y, unsigned int showfilenameonly)
{
	static unsigned int id3order_fe[6] = { I3I_ARTIST, I3I_TITLE, I3I_GENRE, I3I_ALBUM, I3I_YEAR, I3I_COMMENT };
	static unsigned int id3textlen[6] = { 1, 0, 7, 6, 9, 7 };
	static char *id3text[6] = { ":", "", " Album:", " Year:", " Comment:", " Genre:" };
...
void set_mousepos_on_elevator(struct mainvars *mvp, unsigned int mousey)
{
	if(desktopmode & DTM_EDIT_ELEVATOR) {
		...
		if(lines > 0 && remainlines > 0) {
			...
			else if(psi->editorhighline > (pei + lines))
	...
	for(side = 0; side < PLAYLIST_MAX_SIDES; side++, psi++, edp++) {
		...
		if((psi->editsidetype & PLT_ENABLED) && (psi->lastentry >= psi->firstentry)) {
			...
			for(; j <= displayed_rows; j++, lineposy++, pei++) {
				...
				} else if((desktopmode & DTM_EDIT_SHOWDIRDATE) && (lengthcol > 29)
						  && ((pei->entrytype == DFT_SUBDIR) || (pei->entrytype == DFT_SUBLIST) || (pei->entrytype < DFTM_DFT) || (pei->entrytype >= DFT_AUDIOFILE))) {
...
static unsigned int set_editorline_color(struct playlist_side_info *psi, struct playlist_entry_info *pei)
{
	...
	else if((psi == mvp->psip) && (GET_HFT(pei->entrytype) != HFT_DFT) && (pei->infobits & PEIF_ENABLED)) {
...
void display_bufpos_int08(struct mainvars *mvp)
{
	...
	if(displaymode & DISP_FULLSCREEN) {
		...
		else if(displaymode & DISP_NOFULLEDIT)
