...
static void display_textwin_addressmap_window_shadow(textwin_window_t *tw)
{
 ...
 for(y=0;y<sy;y++){
  unsigned long *map=textwin_address_map+(tw->ypos+y+TEXTWIN_WINDOW_SHADOW_Y)*textscreen_maxx*TEXTSCREEN_BYTES_PER_CHAR+(tw->xpos+TEXTWIN_WINDOW_SHADOW_X)*TEXTSCREEN_BYTES_PER_CHAR;
  unsigned int x=sx;
  do{
...
static void display_textwin_reset_window(textwin_window_t *tw)
{
 ...
 if(it && ic){
  do{
   if(it->data){
    switch(it->type){
     case TEXTWIN_LINETYPE_SWITCH:sl=it->data;if(sl->switchtext) free(sl->switchtext);
     case TEXTWIN_LINETYPE_MSG_ALLOCATED:
     case TEXTWIN_LINETYPE_MSG_EDIT     :free(it->data);
    }
   }
   it++;
  }while(--ic);
...
static unsigned int textwin_item_step(textwin_window_t *tw,int step,unsigned int modify_y)
{
 ...
 do{
  ...
  ok=0;
  switch(modify_y){
   case 0:if(it->ypos==last_y) ok=1;break; // left/right arrows
   case 1:ok=1;break;                      // tab
   case 2:if(it->ypos!=last_y) ok=1;break; // up/down arrows
  }
...
static unsigned int textwin_item_select(textwin_window_t *tw,unsigned int keycode)
{
 ...
 switch(keycode){
  ...
  case 0x48e0:tw->item_selected=textwin_item_step(tw,-1,2);goto it_disp;
  case 0x5000: // down arrows
  case 0x50e0:tw->item_selected=textwin_item_step(tw,+1,2);goto it_disp;
  case 0x0f09:tw->item_selected=textwin_item_step(tw,+1,1);goto it_disp; // tab
 }
 it=tw->item_first+tw->item_selected;
 switch(it->type){
  case TEXTWIN_LINETYPE_MSG_EDIT:
  case TEXTWIN_LINETYPE_SWITCH:return 0;
 }
 switch(keycode){
  case 0x4b00: // left arrows
  case 0x4be0:tw->item_selected=textwin_item_step(tw,-1,0);break;
  case 0x4d00: // right arrows
  case 0x4de0:tw->item_selected=textwin_item_step(tw,+1,0);break;
  default:return 0;
 }
it_disp:
...
static unsigned int textwin_line_edit(textwin_window_t *tw,unsigned int keycode)
{
 ...
 if(it->type==TEXTWIN_LINETYPE_MSG_EDIT){
  ...
  switch(keycode){
   ...
   case 0x52e0:if((el->flags&TEXTWIN_EDITLINEFLAG_FIRSTKEY) && !(el->flags&TEXTWIN_EDITFLAG_OVERWRITE))
               ...
               goto le_disp;
   case 0x4b00: // left arrows
   case 0x4be0:if(el->line_data_pos>0)
                el->line_data_pos--;
               else{
   ...
   case 0x47e0:el->line_data_pos=0;
               goto le_disp;
   case 0x4f00: // end keys
   case 0x4fe0:el->line_data_pos=el->line_data_len;
               goto le_disp;
   case 0x0e08:if(el->line_data_pos && el->line_data_len){ // backspace (clear back)
   ...
   case 0x53e0:if(el->line_data_len && (el->line_data_pos<el->line_data_len)){
...
void *display_textwin_openwindow_items(void *tw_update,unsigned int xpos,unsigned int ypos,unsigned int minxsize)
{
 ...
 do{
  ...
  it+=aligned_items;ic+=aligned_items;
 }while(ic<tw->nb_items);
...
static int textwin_additem_msg(void *tw_update,unsigned int flags,
 unsigned int xpos,int ypos,char *msg,unsigned int type)
{
 textwin_window_t *tw=tw_update;
 int itemnum=-1;
 char *next;
...
int display_textwin_additem_msg_static(void *tw_update,unsigned int flags,
 unsigned int xpos,int ypos,char *msg)
{
 return textwin_additem_msg(tw_update,flags,xpos,ypos,msg,TEXTWIN_LINETYPE_MSG_STATIC);
}

int display_textwin_additem_msg_alloc(void *tw_update,unsigned int flags,
 unsigned int xpos,int ypos,char *msg)
{
...
void display_textwin_additem_editline(void *tw_update,unsigned int iflags,unsigned int eflags,
 unsigned int xpos,int ypos,unsigned int minxsize,char *strbuf,unsigned int bufsize)
{
 textwin_window_t *tw=tw_update;
 textwin_item_t *it;
 textwin_editline_t *el;
...
void display_textwin_additem_buttons(void *tw_update,unsigned int flags,
 unsigned int xpos,int ypos,display_textwin_button_t *buttons,
 display_textwin_button_t *selected_button)
{
 textwin_window_t *tw=tw_update;
...
void *display_textwin_openwindow_buttons(void *tw,unsigned int flags,
 char *headtext,char *msg,void *buttonhandfunc,void *buttonhanddata,
 display_textwin_button_t *buttons)
{
