			...
			if(mdep && (mdep->entry_structure_version == MPXPLAY_DLLENTRY_STRUCTURE_VERSION) && mdep->module_entries[0]) {
				for(m = 0; m < MPXPLAY_DLLENTRY_MAX_MODULES; m++) {
					...
					fprintf(stdout, "\n module: mj:%4.4X mi:%4.4X mn:%4s mv:%4.4X mp:%8.8X",
							mdep->module_entries[m]->moduletype_major, mdep->module_entries[m]->moduletype_minor, mdep->module_entries[m]->modulename_minor,
							mdep->module_entries[m]->module_structure_version, mdep->module_entries[m]->module_callpoint);
#endif
				}
			}
		}
		...
		for(i = 0; i < dllfounds_entries; i++, d++) {
			...
			do {
				...
				snprintf(sout, sizeof(sout), "  module: type:%4.4X (%-14s)  sname:%-8s  sver:%4.4X %s",
						 (*mmep)->moduletype_major, mts->name, (*mmep)->modulename_minor, (*mmep)->module_structure_version, (((*mmep)->module_structure_version != mts->ver) ? "not supp" : ""));
				pds_textdisplay_printf(sout);
				mmep++;
			} while(*mmep);
		}
...
long newfunc_dllload_winlib_callfunc(struct pds_win32dllcallfunc_t *func, void *data1, void *data2, void *data3)
{
	...
	switch (func->argnum) {
	case 0:
		return dllload_call_proc_stackbased_arg0(func->funcptr);
	case 1:
		return dllload_call_proc_stackbased_arg1(func->funcptr, data1);
	case 2:
		return dllload_call_proc_stackbased_arg2(func->funcptr, data1, data2);
	default:
		return dllload_call_proc_stackbased_arg3(func->funcptr, data1, data2, data3);
	}
	return 0;
}
