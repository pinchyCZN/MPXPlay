...
void newfunc_cpu_init(void)
{
 ...
 if(ebx == 0x68747541 && edx == 0x69746e65 && ecx == 0x444d4163){ // AMD
  ...
  if(ebx == 0x746e6543 && edx == 0x48727561 && ecx == 0x736c7561){ //  CentaurHauls
   if(cap_ext & (1<<24)) // VIA C3
    cap_mm |= CPU_X86_MMCAP_MMXEXT;
  }else
   if(ebx == 0x69727943 && edx == 0x736e4978 && ecx == 0x64616574){
    if(eax>=2)
     if(cap_ext & (1<<24))
      cap_mm |= CPU_X86_MMCAP_MMXEXT;
   }
 ...
 fprintf(stdout,"cpu features: MMX:%d MMX2(iSSE):%d SSE:%d SSE2:%d 3dNow:%d 3dNow2:%d\n\n"
  ,((cap_mm&CPU_X86_MMCAP_MMX)? 1:0),((cap_mm&CPU_X86_MMCAP_MMXEXT)? 1:0),((cap_mm&CPU_X86_MMCAP_SSE)? 1:0),
  ((cap_mm&CPU_X86_MMCAP_SSE2)? 1:0),((cap_mm&CPU_X86_MMCAP_3DNOW)? 1:0),((cap_mm&CPU_X86_MMCAP_3DNOWEXT)? 1:0));
...
static unsigned int pds_cpu_mtrr_setmode(unsigned long phys_ptr,unsigned long size_kb,unsigned int mode)
{
 ...
 union char_to_int64_t{ unsigned char c[8]; mpxp_int64_t i;} fix16k;
