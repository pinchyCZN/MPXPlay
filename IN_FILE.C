...
static unsigned int infile_checkhead_by_ext(struct frame *frp,struct mpxplay_diskdrive_data_s *mdds,struct mpxplay_infile_func_s *infilefuncs,char *filename,char *extension,unsigned int *extfound);
...
int mpxplay_infile_decode(struct mpxplay_audioout_info_s *aui)
{
 ...
 for(fileselect=0,frp=frp0;fileselect<=((crossfadepart==CROSS_FADE)? 1:0);fileselect++,frp++){
  ...
  if(frp->pcmdec_leftsamples){
   ...
   //else
    frp->pcmdec_leftsamples-=MIXER_conversion(aui,adi,frp,(PCM_CV_TYPE_S *)(frp->pcmdec_buffer+(frp->pcmdec_storedsamples-frp->pcmdec_leftsamples)*adi->bytespersample),frp->pcmdec_leftsamples);

   frp->frameNum++;
   frp->framecounter++;

   mpxplay_debugf(MPXPLAY_DEBUG_OUTPUT,"get    ss:%4d ls:%4d bsc:%4d %8.8X ",frp->pcmout_storedsamples,
     frp->pcmdec_leftsamples,frp->pcmout_blocksize,frp->pcmout_buffer);
  }
 }
...
static unsigned long infile_get_header_autodetect(struct frame *frp,struct mpxplay_diskdrive_data_s *mdds,char *filename)
{
 ...
 for(i=0;i<=LAST_INFILE_FUNCNUM;i++){
  if(all_infile_funcs[i]->detect){
   ...
   switch(retcode){
    case MPXPLAY_ERROR_INFILE_FILEOPEN:return 0;
   ...
   if(MPXPLAY_TAGTYPE_GET_SUPPORT(miis->standard_id3tag_support)){ // common/standard id3tag write
    ...
    if(!is_id3v1 && !is_id3v2 && !is_apetag){ // create new tag
     switch(MPXPLAY_TAGTYPE_GET_PRIMARY(miis->standard_id3tag_support)){
      case MPXPLAY_TAGTYPE_ID3V1 :id3v1_retcode=mpxplay_tagging_id3v1_put(fbfs,frp,miis,id3ip,mpxplay_textconv_funcs);break;
      case MPXPLAY_TAGTYPE_ID3V2 :id3v2_retcode=mpxplay_tagging_id3v2_put(fbfs,frp,miis,id3ip,mpxplay_textconv_funcs);break;
      case MPXPLAY_TAGTYPE_APETAG:apetag_retcode=mpxplay_tagging_apetag_put(fbfs,frp,miis,id3ip,mpxplay_textconv_funcs);break;
