...
static if(!is (typeof(DR_FLAC_BUFFER_SIZE)))
	enum DR_FLAC_BUFFER_SIZE = 4096;

// Check if we can enable 64-bit optimizations.
//version = DRFLAC_64BIT;

version(DRFLAC_64BIT) alias drflac_cache_t = ulong; else
	alias                   drflac_cache_t = uint;
...
//// Endian Management ////
version(LittleEndian) enum drflac__is_little_endian = true; else
	enum drflac__is_little_endian = false;
...
bool drflac__reload_l1_cache_from_l2(drflac_bs* bs)
{
	...
	if(alignedL1LineCount > 0){
		...
		for(size_t i = alignedL1LineCount; i > 0; --i)
			bs.cacheL2.ptr[i - 1 + offset] = bs.cacheL2.ptr[i - 1];
	...
	if(bitsToSeek <= mixin(DRFLAC_CACHE_L1_BITS_REMAINING !"bs")){
		...
		if(wholeBytesRemaining > 0){
			...
			if(wholeCacheLinesRemaining < mixin(DRFLAC_CACHE_L2_LINES_REMAINING !"bs")){
				...
				if(wholeBytesRemaining > 0){
					if(!bs.rs.seek(cast(int)wholeBytesRemaining,drflac_seek_origin_current))
						return false;
...
bool drflac__seek_to_byte(drflac_bs* bs,ulong offsetFromStart)
{
	...
	if(offsetFromStart > 0x7FFFFFFF){
		...
		if(!bs.rs.seek(0x7FFFFFFF,drflac_seek_origin_start))
			return false;
		bytesRemaining -= 0x7FFFFFFF;
		while(bytesRemaining > 0x7FFFFFFF){
			if(!bs.rs.seek(0x7FFFFFFF,drflac_seek_origin_current))
				return false;
			bytesRemaining -= 0x7FFFFFFF;
		}
		if(bytesRemaining > 0){
			if(!bs.rs.seek(cast(int)bytesRemaining,drflac_seek_origin_current))
				return false;
		}
	} else {
		if(!bs.rs.seek(cast(int)offsetFromStart,drflac_seek_origin_start))
			return false;
	}
...
bool drflac__read_utf8_coded_number(drflac_bs* bs,ulong* pNumberOut)
{
	...
	int byteCount = 1;
	if((utf8.ptr[0] & 0xE0) == 0xC0)
		byteCount = 2;
	else if((utf8.ptr[0] & 0xF0) == 0xE0)
		byteCount = 3;
	else if((utf8.ptr[0] & 0xF8) == 0xF0)
		byteCount = 4;
	else if((utf8.ptr[0] & 0xFC) == 0xF8)
		byteCount = 5;
	else if((utf8.ptr[0] & 0xFE) == 0xFC)
		byteCount = 6;
	else if((utf8.ptr[0] & 0xFF) == 0xFE)
		byteCount = 7;
	else {
		*pNumberOut = 0; return false;
	}                                                     // Bad UTF-8 encoding.
...
bool drflac__decode_samples__fixed(drflac_bs* bs,uint blockSize,uint bitsPerSample,ubyte lpcOrder,int* pDecodedSamples)
{
	...
	if(!drflac__decode_samples_with_residual(bs,bitsPerSample,blockSize,lpcOrder,0,lpcCoefficientsTable.ptr[lpcOrder].ptr,pDecodedSamples))
		return false;
...
bool drflac__read_next_frame_header(drflac_bs* bs,ubyte streaminfoBitsPerSample,drflac_frame_header* header)
{
	...
	if(syncCode != 0x3FFE)
		return false;                                               // TODO: Try and recover by attempting to seek to and read the next frame?
...
bool drflac__read_streaminfo(ref ReadStruct rs,drflac_streaminfo* pStreamInfo)
{
	...
	if(rs.read(&blockSizes,4) != 4)
		return false;
	// min/max frame size.
	ulong frameSizes = 0;
	if(rs.read(&frameSizes,6) != 6)
		return false;
	// Sample rate, channels, bits per sample and total sample count.
	ulong importantProps;
	if(rs.read(&importantProps,8) != 8)
		return false;
	// MD5
	ubyte[16] md5;
	if(rs.read(md5.ptr,md5.sizeof) != md5.sizeof)
		return false;
...
bool drflac__read_and_decode_metadata(drflac* pFlac,scope drflac_meta_proc onMeta,void* pUserDataMD)
{
	...
	for(;; ){
		...
		ubyte blockType;
		uint  blockSize;
		if(!drflac__read_and_decode_block_header(pFlac.bs.rs,&isLastBlock,&blockType,&blockSize))
			return false;
		runningFilePos += 4;
		...
		case DRFLAC_METADATA_BLOCK_TYPE_APPLICATION:
			if(onMeta){
				...
				if(pFlac.bs.rs.read(pRawData,blockSize) != blockSize)
					return false;
		...
		case DRFLAC_METADATA_BLOCK_TYPE_PADDING:
			if(onMeta){
				...
				if(!pFlac.bs.rs.seek(blockSize,drflac_seek_origin_current))
					return false;
...
bool drflac__init_private__native(drflac_init_info* pInit,ref ReadStruct rs,scope drflac_meta_proc onMeta,void* pUserDataMD)
{
	...
	ubyte blockType;
	uint  blockSize;
	if(!drflac__read_and_decode_block_header(rs,&isLastBlock,&blockType,&blockSize))
		return false;
...
uint drflac_ogg__get_page_body_size(drflac_ogg_page_header* pHeader)
{
	...
	for(int i = 0; i < pHeader.segmentCount; ++i)
		pageBodySize += pHeader.segmentTable.ptr[i];
	return pageBodySize;
}
...
{
	if(rs.read(&pHeader.structureVersion,1) != 1 || pHeader.structureVersion != 0)
		return false;                                                                                                          // Unknown structure version. Possibly corrupt stream.
	if(rs.read(&pHeader.headerType,1) != 1)
		return false;
	if(rs.read(&pHeader.granulePosition,8) != 8)
		return false;
	if(rs.read(&pHeader.serialNumber,4) != 4)
		return false;
	if(rs.read(&pHeader.sequenceNumber,4) != 4)
		return false;
	if(rs.read(&pHeader.checksum,4) != 4)
		return false;
	if(rs.read(&pHeader.segmentCount,1) != 1 || pHeader.segmentCount == 0)
		return false;                                                                                                  // Should not have a segment count of 0.
	if(rs.read(&pHeader.segmentTable,pHeader.segmentCount) != pHeader.segmentCount)
		return false;
	if(pHeaderSize)
		*pHeaderSize = (27 + pHeader.segmentCount);
	return true;
}
...
{
	ubyte[4] id;
	if(rs.read(id.ptr,4) != 4)
		return false;
	if(id.ptr[0] != 'O' || id.ptr[1] != 'g' || id.ptr[2] != 'g' || id.ptr[3] != 'S')
		return false;
...
bool drflac_oggbs__seek_physical(drflac_oggbs* oggbs,ulong offset,drflac_seek_origin origin)
{
	if(origin == drflac_seek_origin_start){
		if(offset <= 0x7FFFFFFF){
			if(!oggbs.rs.seek(cast(int)offset,drflac_seek_origin_start))
				return false;
			oggbs.currentBytePos = offset;
			return true;
		} else {
			if(!oggbs.rs.seek(0x7FFFFFFF,drflac_seek_origin_start))
				return false;
			oggbs.currentBytePos = offset;
		...
		while(offset > 0x7FFFFFFF){
			if(!oggbs.rs.seek(0x7FFFFFFF,drflac_seek_origin_current))
				return false;
			oggbs.currentBytePos += 0x7FFFFFFF;
			offset               -= 0x7FFFFFFF;
		}
		if(!oggbs.rs.seek(cast(int)offset,drflac_seek_origin_current))
...
bool drflac_oggbs__goto_next_page(drflac_oggbs* oggbs)
{
	...
	for(;; ){
		uint headerSize;
		if(!drflac_ogg__read_page_header(oggbs.rs,&header,&headerSize))
			return false;
...
bool drflac__init_private__ogg(drflac_init_info* pInit,ref ReadStruct rs,scope drflac_meta_proc onMeta,void* pUserDataMD)
{
	...
	uint                   headerSize;
	if(!drflac_ogg__read_page_header_after_capture_pattern(rs,&header,&headerSize))
		return false;
	...
	for(;; ){
		...
		if(pageBodySize == 51){                                 // 51 = the lacing value of the FLAC header packet.
			...
			if(packetType == 0x7F){
				...
				if(sig.ptr[0] == 'F' && sig.ptr[1] == 'L' && sig.ptr[2] == 'A' && sig.ptr[3] == 'C'){
					...
					if(rs.read(mappingVersion.ptr,2) != 2)
						return false;

					if(mappingVersion.ptr[0] != 1)
						return false;                                                                                                 // Only supporting version 1.x of the Ogg mapping.

					// The next 2 bytes are the non-audio packets, not including this one. We don't care about this because we're going to
					// be handling it in a generic way based on the serial number and packet types.
					if(!rs.seek(2,drflac_seek_origin_current))
						return false;
					...
					if(!rs.seek(bytesRemainingInPage,drflac_seek_origin_current))
						return false;
				}
			} else {
			...
			if(!rs.seek(pageBodySize,drflac_seek_origin_current))
				return false;
		}
		...
		if(!drflac_ogg__read_page_header(rs,&header,&headerSize))
			return false;
		pInit.runningFilePos += headerSize;
	}
...
{
	ubyte[4] id;
	if(pInit.rs.read(id.ptr,4) != 4)
		return false;
	if(id.ptr[0] == 'f' && id.ptr[1] == 'L' && id.ptr[2] == 'a' && id.ptr[3] == 'C')
		return drflac__init_private__native(pInit,pInit.rs,onMeta,pUserDataMD);
//#ifndef DR_FLAC_NO_OGG
	if(id.ptr[0] == 'O' && id.ptr[1] == 'g' && id.ptr[2] == 'g' && id.ptr[3] == 'S')
		return drflac__init_private__ogg(pInit,pInit.rs,onMeta,pUserDataMD);
//#endif
...
public void drflac_close(drflac* pFlac)
{
	...
	if(pFlac.bs.stdio)
		drflac__close_file_handle(cast(drflac_file)pFlac.bs.rs.pUserData);
...
//#ifndef DR_FLAC_NO_OGG
	...
	if(pFlac.container == drflac_container_ogg){
		...
		if(oggbs.stdio)
			drflac__close_file_handle(cast(drflac_file)oggbs.rs.pUserData);
	}
//#endif
	free(pFlac);
}
