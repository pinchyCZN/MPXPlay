...
static void mpc_copy_seek_to_mpci(struct mpc_decoder_data *mpci)
{
 ...
 pds_memcpy((char *)mpci->bitstream,(char *)seek_helper->bitstream,MPC_BITSTREAM_BYTESIZE);
...
static void mpc_copy_mpci_to_seek(struct mpc_decoder_data *mpci)
{
 unsigned int intsoundcntrl_save;
 MPXPLAY_INTSOUNDDECODER_DISALLOW
 pds_memcpy((char *)seek_helper->bitstream,(char *)mpci->bitstream,MPC_BITSTREAM_BYTESIZE);
