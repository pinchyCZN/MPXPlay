 ...
 if(fileoffset>0){                // successfull file positioning
  ...
  if(id3ip[I3I_TRACKNUM] && id3ip[I3I_TRACKNUM][0]){       // ID3v1.1
   ...
   sprintf(strtemp,"TAG%-30.30s%-30.30s%-30.30s%-4.4s%-28.28s%c%c%c",
                   ...
                   tracknum,
                   tagging_id3v1_genre_to_index(id3ip[I3I_GENRE]));
  }else{                                                   // ID3v1.0
...
typedef struct id3v2x_one_frame_handler_s{
 ...
 long (*frame_reader)(struct id3v2x_main_data_s *imds,struct id3v2x_one_frame_data_s *framedata,struct mpxplay_filehand_buffered_func_s *fbfs,void *fbds,long datalen,char **id3ip,struct mpxplay_textconv_func_s *mpxplay_textconv_funcs);
 long (*frame_writer)(struct id3v2x_main_data_s *imds,struct id3v2x_one_frame_data_s *framedata,struct mpxplay_filehand_buffered_func_s *fbfs,void *fbds,char *id3ip,struct mpxplay_textconv_func_s *mpxplay_textconv_funcs);
...
static long mpx_id3v2x_textconv_utf_to_char(char *databuf,long datalen,unsigned int text_enc_type,struct mpxplay_textconv_func_s *mpxplay_textconv_funcs)
{
 if(((*(mpxplay_textconv_funcs->control))&ID3TEXTCONV_UTF_AUTO) && databuf){
  switch(text_enc_type){
   case ID3V2_TEXTENCTYPE_UTF16LE:datalen=mpxplay_textconv_funcs->utf16LE_to_char(databuf,datalen);mpxplay_textconv_funcs->convdone=ID3TEXTCONV_UTF16;break;
   case ID3V2_TEXTENCTYPE_UTF16BE:datalen=mpxplay_textconv_funcs->utf16BE_to_char(databuf,datalen);mpxplay_textconv_funcs->convdone=ID3TEXTCONV_UTF16;break;
   case ID3V2_TEXTENCTYPE_UTF8   :datalen=mpxplay_textconv_funcs->utf8_to_char(databuf,datalen);mpxplay_textconv_funcs->convdone=ID3TEXTCONV_UTF8;break;
  }
 }
 return datalen;
}

static long mpx_id3v2x_tagread_text(struct id3v2x_main_data_s *imds,struct id3v2x_one_frame_data_s *framedata,struct mpxplay_filehand_buffered_func_s *fbfs,void *fbds,long datalen,char **id3ip,struct mpxplay_textconv_func_s *mpxplay_textconv_funcs)
{
 if(datalen<2)
  return 0;
...
static long mpx_id3v2x_tagread_txxx(struct id3v2x_main_data_s *imds,struct id3v2x_one_frame_data_s *framedata,struct mpxplay_filehand_buffered_func_s *fbfs,void *fbds,long datalen,char **id3ip,struct mpxplay_textconv_func_s *mpxplay_textconv_funcs)
{
...
static long mpx_id3v2x_tagread_tcon(struct id3v2x_main_data_s *imds,struct id3v2x_one_frame_data_s *framedata,struct mpxplay_filehand_buffered_func_s *fbfs,void *fbds,long datalen,char **id3ip,struct mpxplay_textconv_func_s *mpxplay_textconv_funcs)
{
 datalen=mpx_id3v2x_tagread_text(imds,framedata,fbfs,fbds,datalen,id3ip,mpxplay_textconv_funcs);
 if((datalen>0) && imds->id3p){
  char *id3p=imds->id3p;
...
static long mpx_id3v2x_tagread_comm(struct id3v2x_main_data_s *imds,struct id3v2x_one_frame_data_s *framedata,struct mpxplay_filehand_buffered_func_s *fbfs,void *fbds,long datalen,char **id3ip,struct mpxplay_textconv_func_s *mpxplay_textconv_funcs)
{
...
static unsigned int mpx_id3v2x_textconv_char_to_utf(char *destbuf,unsigned int buflen,char *srcbuf,unsigned int datalen,unsigned int text_enc_type,struct mpxplay_textconv_func_s *mpxplay_textconv_funcs)
{
 if((*(mpxplay_textconv_funcs->control))&ID3TEXTCONV_UTF_AUTO){
  switch(text_enc_type){
   ...
   case ID3V2_TEXTENCTYPE_UTF8:
    ...
    // !!! trailing zeroes are not written
    break;
   default:pds_memcpy(destbuf,srcbuf,datalen);break;
  }
 }else
...
static long mpx_id3v2x_tagwrite_text(struct id3v2x_main_data_s *imds,struct id3v2x_one_frame_data_s *framedata,struct mpxplay_filehand_buffered_func_s *fbfs,void *fbds,char *id3ip,struct mpxplay_textconv_func_s *mpxplay_textconv_funcs,unsigned int comment)
{
...
static long mpx_id3v2x_tagwrite_txxx(struct id3v2x_main_data_s *imds,struct id3v2x_one_frame_data_s *framedata,struct mpxplay_filehand_buffered_func_s *fbfs,void *fbds,char *id3ip,struct mpxplay_textconv_func_s *mpxplay_textconv_funcs)
{
 return mpx_id3v2x_tagwrite_text(imds,framedata,fbfs,fbds,id3ip,mpxplay_textconv_funcs,0);
}

static long mpx_id3v2x_tagwrite_tcon(struct id3v2x_main_data_s *imds,struct id3v2x_one_frame_data_s *framedata,struct mpxplay_filehand_buffered_func_s *fbfs,void *fbds,char *id3ip,struct mpxplay_textconv_func_s *mpxplay_textconv_funcs)
{
...
static long mpx_id3v2x_tagwrite_comm(struct id3v2x_main_data_s *imds,struct id3v2x_one_frame_data_s *framedata,struct mpxplay_filehand_buffered_func_s *fbfs,void *fbds,char *id3ip,struct mpxplay_textconv_func_s *mpxplay_textconv_funcs)
{
...
static id3v2x_one_frame_handler_s id3v23_supported_frames[]=
...
static id3v2x_one_frame_handler_s id3v24_supported_frames[]=
{{"TPE1",I3I_ARTIST  ,&mpx_id3v2x_tagread_txxx,&mpx_id3v2x_tagwrite_txxx},
 {"TIT2",I3I_TITLE   ,&mpx_id3v2x_tagread_txxx,&mpx_id3v2x_tagwrite_txxx},
 {"TALB",I3I_ALBUM   ,&mpx_id3v2x_tagread_txxx,&mpx_id3v2x_tagwrite_txxx},
 {"TYER",I3I_YEAR    ,&mpx_id3v2x_tagread_txxx,&mpx_id3v2x_tagwrite_txxx},
...
static unsigned int mpx_id3v2_read_main(struct id3v2x_main_data_s *imds,struct mpxplay_filehand_buffered_func_s *fbfs,void *fbds,char **id3ip,struct mpxplay_textconv_func_s *mpxplay_textconv_funcs)
{
...
char *mpxplay_tagging_id3v2_get(struct mpxplay_filehand_buffered_func_s *fbfs,void *fbds,struct mpxplay_infile_info_s *miis,char **id3ip,char *id3p,struct mpxplay_textconv_func_s *mpxplay_textconv_funcs)
{
 struct id3v2x_main_data_s imds;
 pds_memset((void *)&imds,0,sizeof(imds));
 imds.control=ID3V2_CONTROL_READ;
...
static char *ape_tag_read_apetag(struct mpxplay_filehand_buffered_func_s *fbfs,void *fbds,struct APETag_s *apetag,struct mpxplay_infile_info_s *miis,char **id3ip,char *id3p,struct mpxplay_textconv_func_s *mpxplay_textconv_funcs)
{
...
char *mpxplay_tagging_apetag_get(struct mpxplay_filehand_buffered_func_s *fbfs,void *fbds,struct mpxplay_infile_info_s *miis,char **id3ip,char *id3p,struct mpxplay_textconv_func_s *mpxplay_textconv_funcs)
{
 struct APETag_s apetag;
