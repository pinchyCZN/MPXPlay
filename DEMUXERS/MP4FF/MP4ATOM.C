...
static uint32_t mp4ff_read_alac(mp4ff_t * f, int32_t size)
{
	...
	size -= 4;
	p_track->sampleRate = mp4ff_read_int16(f);
	size -= 2;
	mp4ff_read_int16(f);
	size -= 2;
...
static int32_t mp4ff_read_stsd(mp4ff_t * f)
{
	...
	for(i = 0; i < p_track->stsd_entry_count; i++) {
		...
		case ATOM_MP4A:
			...
			break;
		case ATOM_MP4V:
			p_track->type = TRACK_VIDEO;
			break;
		case ATOM_MP4S:
			p_track->type = TRACK_SYSTEM;
			break;
		case ATOM_ALAC:
			p_track->type = TRACK_AUDIO;
...
int32_t mp4ff_atom_read(mp4ff_t * f, const int32_t size, const uint8_t atom_type)
{
	...
	case ATOM_STSZ:
		mp4ff_read_stsz(f);
		break;					// sample size box
	case ATOM_STTS:
		mp4ff_read_stts(f);
		break;					// time to sample box
	case ATOM_CTTS:
		mp4ff_read_ctts(f);
		break;					// composition offset box
	case ATOM_STSC:
		mp4ff_read_stsc(f);
		break;					// sample to chunk box
	case ATOM_STCO:
		mp4ff_read_stco(f);
		break;					// chunk offset box
	case ATOM_STSD:
		mp4ff_read_stsd(f);
		break;					// sample description box
	case ATOM_MDHD:
		mp4ff_read_mdhd(f);
		break;					// track header
	case ATOM_META:
		mp4ff_read_meta(f, size);
		break;					// iTunes Metadata box
	}
