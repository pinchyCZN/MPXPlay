...
**
** $Id: mp4util.c,v 1.20 2005/01/12 00:00:00 PDSoft Exp $
**/
...
{
	long ret = f->stream->seek(f->stream->user_data, (long)position, SEEK_SET);

	if(ret < 0)
		return ret;

	if(ret != (long)position)
		return -1;
...
uint64_t mp4ff_read_int64(mp4ff_t * f)
{
	...
	mp4ff_read_data(f, (int8_t *) & data[0], 8);
