...
//#define WMA_LOG(...) av_log(NULL, AV_LOG_DEBUG, __VA_ARGS__)
#define WMA_LOG()
...
typedef struct WMADecodeContext {
	...
	float decode_block_window_tmp[BLOCK_MAX_SIZE * 2];
	FFTSample decode_block_output_tmp[BLOCK_MAX_SIZE * 2] __attribute__ ((aligned(16)));
...
static int wma_decode_init(AVCodecContext * avctx)
{
	...
	avctx->sample_fmt = SAMPLE_FMT_F16;
	...
	if(s->version == 1 && avctx->extradata_size >= 4) {
		...
		flags1 = extradata[0] | (extradata[1] << 8) | (extradata[2] << 16) | (extradata[3] << 24);
	...
	if(s->sample_rate <= 16000) {
		s->frame_len_bits = 9;
	} else if(s->sample_rate <= 22050 || (s->sample_rate <= 32000 && s->version == 1)) {
		s->frame_len_bits = 10;
	} else {
		s->frame_len_bits = 11;
	}
...
#if 0
			tprintf("%5d: coefs_end=%d high_band_start=%d nb_high_bands=%d: ", s->frame_len >> k, s->coefs_end[k], s->high_band_start[k], s->exponent_high_sizes[k]);
...
#ifdef TRACE
	{
		...
		for(i = 0; i < s->nb_block_sizes; i++) {
			tprintf("%5d: n=%2d:", s->frame_len >> i, s->exponent_sizes[i]);
	...
	if(s->use_noise_coding) {
...
#else
		{
			...
			float norm;
			seed = 1;
			norm = (1.0 / (float)((int64_t) 1 << 31)) * sqrt(3) * s->noise_mult;
			for(i = 0; i < NOISE_TAB_SIZE; i++) {
				seed = seed * 314159 + 1;
...
#endif
		init_vlc(&s->hgain_vlc, 9, sizeof(hgain_huffbits), hgain_huffbits, 1, 1, hgain_huffcodes, 2, 2, 0);
	}

	if(s->use_exp_vlc) {
		init_vlc(&s->exp_vlc, 9, sizeof(scale_huffbits), scale_huffbits, 1, 1, scale_huffcodes, 4, 4, 0);
	} else {
	...
	init_coef_vlc(&s->coef_vlc[0], &s->run_table[0], &s->level_table[0], &coef_vlcs[coef_vlc_table * 2]);
	init_coef_vlc(&s->coef_vlc[1], &s->run_table[1], &s->level_table[1], &coef_vlcs[coef_vlc_table * 2 + 1]);
	return 0;
}
...
{
	float val_max = 0, *costab = s->lsp_cos_table;
	...
	do {
		float p = 0.5f;
		float q = 0.5f;
		float w = *costab++, *lspp = lsp;
		unsigned int j = NB_LSP_COEFS / 2;
		do {
			q *= w - lspp[0];
			p *= w - lspp[1];
			lspp += 2;
...
static int wma_decode_block(WMADecodeContext * s)
{
	...
	float *window = s->decode_block_window_tmp;
...
//#endif
	...
	if(s->use_variable_block_len) {
		...
		if(s->reset_block_lengths) {
			...
			if(v >= s->nb_block_sizes) {
				WMA_LOG("1. v >= s->nb_block_sizes\n");
				return -1;
			}
			s->prev_block_len_bits = s->frame_len_bits - v;
			v = get_bits(&s->gb, n);
			if(v >= s->nb_block_sizes) {
				WMA_LOG("2. v >= s->nb_block_sizes\n");
				return -1;
			}
	...
	if((s->block_pos + s->block_len) > s->frame_len) {
		WMA_LOG("4. (s->block_pos + s->block_len) > s->frame_len\n");
		return -1;
	}
	...
	if(parse_exponents) {
			...
			if(s->channel_coded[ch]) {
				if(s->use_exp_vlc) {
					if(decode_exp_vlc(s, ch) < 0) {
						WMA_LOG("7. decode_exp_vlc failed \n");
						return -1;
					}
				} else {
					decode_exp_lsp(s, ch);
		...
		for(ch = 0; ch < s->nb_channels; ch++) {
			if(s->channel_coded[ch]) {
				interpolate_array(s->exponents[ch], 1 << s->prev_block_len_bits, s->block_len);
			}
		}
	}
	...
	for(ch = 0; ch < s->nb_channels; ch++) {
		if(s->channel_coded[ch]) {
			...
			int16_t *ptr, *eptr;
			uint16_t *level_table, *run_table;
			...
			for(;;) {
				...
				ptr += run;
				if(ptr >= eptr) {
					WMA_LOG("9. ptr>=eptr code:%d run:%d\n", code, run);
					return -1;
				}
				*ptr++ = level;
	...
	for(ch = 0; ch < s->nb_channels; ch++) {
		if(s->channel_coded[ch]) {
			...
			if(s->use_noise_coding) {
				...
				exp_ptr = exponents + s->high_band_start[bsize] - s->coefs_start;
	...
	for(ch = 0; ch < s->nb_channels; ch++) {
		if(s->channel_coded[ch]) {
			FFTSample *output = s->decode_block_output_tmp;
			...
			n = s->block_len;
			n4 = s->block_len / 2;
			ff_imdct_calc(&s->mdct_ctx[bsize], output, s->coefs[ch], s->mdct_tmp);
...
static int wma_decode_frame(WMADecodeContext * s, float *samples)
{
	int ret, i, n, ch, incr;
	float *ptr;
	float *iptr;
...
//#endif
	...
	/* convert frame to interlaced float out */
	n = s->frame_len;
	incr = s->nb_channels;
	for(ch = 0; ch < s->nb_channels; ch++) {
		ptr = samples + ch;
		...
		memmove(&s->frame_out[ch][0], &s->frame_out[ch][s->frame_len], s->frame_len * sizeof(float));
		/* XXX: suppress this */
		memset(&s->frame_out[ch][s->frame_len], 0, s->frame_len * sizeof(float));
	}
...
static void wma_flush(AVCodecContext * avctx, unsigned int fullclear)
{
	WMADecodeContext *s = avctx->priv_data;
	unsigned int ch;
	...
	if(fullclear)
		for(ch = 0; ch < s->nb_channels; ch++)
			memset(&s->frame_out[ch][0], 0, s->frame_len * sizeof(float));

	//memset(s->mdct_tmp,0,BLOCK_MAX_SIZE*sizeof(FFTSample));
}
