...
void ff_fft_calc_c(FFTContext * s, FFTComplex * z)
{
	...
	j = (np >> 1);
	do {
		BF(p[0].re, p[0].im, p[1].re, p[1].im, p[0].re, p[0].im, p[1].re, p[1].im);
		p += 2;
	} while(--j != 0);
	...
	if(s->inverse) {
		do {
			BF(p[0].re, p[0].im, p[2].re, p[2].im, p[0].re, p[0].im, p[2].re, p[2].im);
			BF(p[1].re, p[1].im, p[3].re, p[3].im, p[1].re, p[1].im, -p[3].im, p[3].re);
			p += 4;
		} while(--j != 0);
	} else {
		do {
	...
	do {
		...
		q = z + nloops;
		for(j = 0; j < nblocks; ++j) {
			BF(p->re, p->im, q->re, q->im, p->re, p->im, q->re, q->im);
