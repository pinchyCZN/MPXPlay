...
int mpxdec_syncinfo(struct mp3_decoder_data *mp3d,unsigned char *bsbuf)
{
 ...
 switch(mp3d->lay){
  case 2:mpxdec_layer2_getstuff(mp3d);
	 break;
  case 3:if(mp3d->lsf)
...
int mpxdec_decode_part1(struct mp3_decoder_data *mp3d,unsigned int chan_mode)
{
 ...
 switch(mp3d->lay){
  case 2:mpxdec_layer2_decode_part1(mp3d);retcode=MPXDEC_ERROR_OK;break;
  case 3:retcode=mpxdec_layer3_decode_part1(mp3d,chan_mode);break;
 }
 return retcode;
}
...
static unsigned int III_get_side_info_1(struct gr_info_s *gr_info,int stereo,int ms_stereo,long sfreq)
{
 ...
 do{
  ...
  if(i&1){ // window-switching flag==1 for block_Type!=0 ; window-sw-flag = 0 for block_type==0
   ...
   gr_info->table_select[1] = i&31; i>>=5;    // 5 bits
   gr_info->table_select[0] = i&31; i>>=5;    // 5 bits
   gr_info->mixed_block_flag= i&1;  i>>=1;    // 1 bit
   ...
   i=getbits_24(12);
   gr_info->count1table_select = i&1; i>>=1;
   if(i&1){ // gr_info->scalefac_scale
    ...
    gr_info->preflag     = i&1;   i>>=3;
    gr_info->full_gain[1]=(i&14); i>>=3;
    ...
    gr_info->preflag     = i&2;   i>>=3;
    gr_info->full_gain[1]=(i&28); i>>=3;
   ...
   gr_info->mapp=map[sfreq][2];
   i=getbits_24(15);
   gr_info->table_select[2]= i&31; i>>=5;
   gr_info->table_select[1]= i&31; i>>=5;
