...
static void CAntiPredictorHigh3600To3700_AntiPredict(int *pInputArray, int *pOutputArray, int NumberOfElements)
{
	...
	for(q = 13; q < NumberOfElements; q++) {
		...
		OP0 =
			(pInputArray[q] - ((bp1 * bm1) >> 8) + ((bp2 * bm2) >> 8) - ((bp3 * bm3) >> 8) - ((bp4 * bm4) >> 8) - ((bp5 * bm5) >> 8) - ((bp6 * bm6) >> 8) - ((bp7 * bm7) >> 8) - ((bp8 * bm8) >> 8) -
			 ((bp9 * bm9) >> 8) + ((bp10 * bm10) >> 8) + ((bp11 * bm11) >> 8) + ((bp12 * bm12) >> 8) + ((bp13 * bm13) >> 8));
...
static void CAntiPredictorHigh3700To3800_AntiPredict(int *pInputArray, int *pOutputArray, int NumberOfElements)
{
	...
	for(; op < &pOutputArray[NumberOfElements]; op++, ip++) {
		...
		*ip =
			Original -
			(((ip[-1] * bm[0]) + (ip[-2] * bm[1]) + (ip[-3] * bm[2]) + (ip[-4] * bm[3]) + (ip[-5] * bm[4]) + (ip[-6] * bm[5]) + (ip[-7] * bm[6]) + (ip[-8] * bm[7]) + (ip[-9] * bm[8]) +
			  (ip[-10] * bm[9]) + (ip[-11] * bm[10]) + (ip[-12] * bm[11]) + (ip[-13] * bm[12]) + (ip[-14] * bm[13]) + (ip[-15] * bm[14]) + (ip[-16] * bm[15])) >> 8);
