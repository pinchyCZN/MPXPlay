...
static const unsigned __int32 K_SUM_MIN_BOUNDARY_OLD[32] =
	...
	1073741824, 2147483648, 0, 0, 0, 0, 0, 0
};
static const unsigned __int32 K_SUM_MAX_BOUNDARY_OLD[32] =
	{ 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536, 131072, 262144, 524288, 1048576, 2097152, 4194304, 8388608, 16777216, 33554432, 67108864, 134217728, 268435456, 536870912, 1073741824,
	2147483648, 0, 0, 0, 0, 0, 0, 0
};
static const unsigned __int32 Powers_of_Two[32] =
...
static const unsigned __int32 Powers_of_Two_Reversed[32] =
	{ 2147483648, 1073741824, 536870912, 268435456, 134217728, 67108864, 33554432, 16777216, 8388608, 4194304, 2097152, 1048576, 524288, 262144, 131072, 65536, 32768, 16384, 8192, 4096, 2048, 1024,
	512, 256, 128, 64, 32, 16, 8, 4, 2, 1
};
static const unsigned __int32 Powers_of_Two_Minus_One_Reversed[33] =
	{ 4294967295, 2147483647, 1073741823, 536870911, 268435455, 134217727, 67108863, 33554431, 16777215, 8388607, 4194303, 2097151, 1048575, 524287, 262143, 131071, 65535, 32767, 16383, 8191, 4095,
	2047, 1023, 511, 255, 127, 63, 31, 15, 7, 3, 1, 0
};
static const unsigned __int32 K_SUM_MAX_BOUNDARY[32] =
...
static unsigned int CUnBitArrayOld_DecodeValue(void *unbitarray_data, enum DECODE_VALUE_METHOD DecodeMethod, int nParam1, int nParam2)
{
	...
	case DECODE_VALUE_METHOD_UNSIGNED_RICE:
		return DecodeValueRiceUnsigned(bita_data, nParam1);
	case DECODE_VALUE_METHOD_X_BITS:
		return bita_data->unbitbas_funcs->DecodeValueXBits(bita_data->unbitbas_datas, nParam1);
	}
	return 0;
}
...
static int DecodeValueNew(struct CUnBitArrayOld_data_s *bita_data, BOOL bCapOverflow)
{
	...
	if(bita_data->K_Sum < K_SUM_MIN_BOUNDARY[bita_data->k]) {
		bita_data->k--;
	} else if(bita_data->K_Sum >= K_SUM_MAX_BOUNDARY[bita_data->k]) {
		bita_data->k++;
	}
