 ...
 for(z = 0; z <= nNumberOfCoefficients; z++){                                                             \
  ...
  aryCoefficientsB[z] = core_data->unbita_funcs->DecodeValue(core_data->unbita_datas,DECODE_VALUE_METHOD_X_BITS,ValueBits,0); \
	...
	case COMPRESSION_LEVEL_EXTRA_HIGH:
		...
		} else if(fileversion < 3600) {
			GET_COEFFICIENTS(3, 5)
				core_data->unbita_funcs->GenerateArray(core_data->unbita_datas, core_data->m_pTempData, Number_of_Elements, nFrameBytes);
			((predoldfunc_eh_0000to3799_t) pAntiPredictor) (core_data->m_pTempData, Input_Array, Number_of_Elements, nNumberOfCoefficients, &aryCoefficientsA[0], &aryCoefficientsB[0]);
		} else if(fileversion < 3700) {
			GET_COEFFICIENTS(3, 6)
				core_data->unbita_funcs->GenerateArray(core_data->unbita_datas, core_data->m_pTempData, Number_of_Elements, nFrameBytes);
			((predoldfunc_eh_0000to3799_t) pAntiPredictor) (core_data->m_pTempData, Input_Array, Number_of_Elements, nNumberOfCoefficients, &aryCoefficientsA[0], &aryCoefficientsB[0]);
		} else if(fileversion < 3800) {
			GET_COEFFICIENTS(3, 6)
				core_data->unbita_funcs->GenerateArray(core_data->unbita_datas, core_data->m_pTempData, Number_of_Elements, nFrameBytes);
			((predoldfunc_eh_0000to3799_t) pAntiPredictor) (core_data->m_pTempData, Input_Array, Number_of_Elements, nNumberOfCoefficients, &aryCoefficientsA[0], &aryCoefficientsB[0]);
		} else {
			core_data->unbita_funcs->GenerateArray(core_data->unbita_datas, core_data->m_pTempData, Number_of_Elements, nFrameBytes);
			((predoldfunc_eh_3800toCurrent_t) pAntiPredictor) (core_data->m_pTempData, Input_Array, Number_of_Elements, core_data->m_bMMXAvailable, fileversion, core_data->IPAdaptFactor,
															   core_data->IPShort);
		}
		break;
	}
...
static int CUnMAC_DecompressFrame(struct CAPEDecompressOld_data_s *dec_data, unsigned char *pOutputData, __int32 FrameIndex)
{
	...
	nBlocks =
		((FrameIndex + 1) >= CAPEDecompressOld_GetInfo((void *)dec_data, APE_INFO_TOTAL_FRAMES, 0, 0)) ? CAPEDecompressOld_GetInfo((void *)dec_data, APE_INFO_FINAL_FRAME_BLOCKS, 0,
																																   0) : CAPEDecompressOld_GetInfo((void *)dec_data,
																																								  APE_INFO_BLOCKS_PER_FRAME, 0, 0);
	if(nBlocks == 0)
		return -1;
	...
	if(CAPEDecompressOld_GetInfo((void *)dec_data, APE_INFO_CHANNELS, 0, 0) == 2) {
		...
		apedec_UnprepareOld(core_data->m_pDataX, core_data->m_pDataY, nBlocks, &unmac_data->m_wfeInput, pOutputData, (unsigned int *)&CRC, (int *)&nSpecialCodes,
							CAPEDecompressOld_GetInfo((void *)dec_data, APE_INFO_FILE_VERSION, 0, 0));
	} else if(CAPEDecompressOld_GetInfo((void *)dec_data, APE_INFO_CHANNELS, 0, 0) == 1) {
		CAPEDecompressOldCore_GenerateDecodedArrays(core_data, nBlocks, nSpecialCodes, FrameIndex);
		apedec_UnprepareOld(core_data->m_pDataX, NULL, nBlocks, &unmac_data->m_wfeInput, pOutputData, (unsigned int *)&CRC, (int *)&nSpecialCodes,
							CAPEDecompressOld_GetInfo((void *)dec_data, APE_INFO_FILE_VERSION, 0, 0));
	}
...
static int CUnMAC_SeekToFrame(struct CAPEDecompressOld_data_s *dec_data, int FrameIndex)
{
	...
	if(GET_FRAMES_START_ON_BYTES_BOUNDARIES(dec_data->m_spAPEInfo)) {
		if((unmac_data->m_LastDecodedFrameIndex == -1) || ((FrameIndex - 1) != unmac_data->m_LastDecodedFrameIndex)) {
			...
			return core_data->unbita_funcs->FillAndResetBitArray(core_data->unbita_datas, CAPEDecompressOld_GetInfo((void *)dec_data, APE_INFO_SEEK_BYTE, FrameIndex, 0) - SeekRemainder,
																 SeekRemainder * 8);
		} else {
