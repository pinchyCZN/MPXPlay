...
static void floor0_free_look(vorbis_look_floor * vlf)
{
	...
	if(look) {
		...
		if(look->linearcosmap) {
			if(look->linearcosmap[0])
				_ogg_free(look->linearcosmap[0]);
			if(look->linearcosmap[1])
				_ogg_free(look->linearcosmap[1]);
...
static void floor0_map_lazy_init(vorbis_dsp_state * vd, vorbis_info_floor * infoX, vorbis_look_floor0 * look, unsigned int W)
{
...
static int floor0_inverse2(vorbis_block * vb, vorbis_look_floor * i, void *memo, ogg_double_t * out)
{
	...
	if(memo) {
		...
		vorbis_lsp_to_curve(out, lsp, look->linearcosmap[vb->W], look->n[vb->W], look->m, amp, (ogg_double_t) info->ampdB);
		return (1);
	}
