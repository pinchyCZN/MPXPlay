			...
			for(m = sbr->f_table_noise[i]; m < sbr->f_table_noise[i + 1]; m++) {
				...
				for(k = 0; k < 2; k++) {
					if((sbr->t_E[ch][l] >= sbr->t_Q[ch][k]) && (sbr->t_E[ch][l + 1] <= sbr->t_Q[ch][k + 1])) {
						adj->Q_mapped[m - sbr->kx][l] = sbr->Q_orig[ch][i][k];
					}
				}
			}
		}
	...
	for(l = 0; l < sbr->L_E[ch]; l++) {
		for(i = 0; i < sbr->N_high; i++) {
			for(m = sbr->f_table_res[HI_RES][i]; m < sbr->f_table_res[HI_RES][i + 1]; m++) {
				...
				if((l >= sbr->l_A[ch]) || ((sbr->bs_add_harmonic_prev[ch][i]) && (sbr->bs_add_harmonic_flag_prev[ch]))) {
					delta_step = 1;
				}

				if(m == (int32_t) ((real_t) (sbr->f_table_res[HI_RES][i + 1] + sbr->f_table_res[HI_RES][i]) / 2.)) {
					adj->S_index_mapped[m - sbr->kx][l] = delta_step * sbr->bs_add_harmonic[ch][i];
				} else {
		...
		for(i = 0; i < sbr->N_high; i++) {
			if(sbr->f[ch][l] == 1) {
				...
				for(k1 = 0; k1 < sbr->N_low; k1++) {
					if((sbr->f_table_res[HI_RES][i] >= sbr->f_table_res[LO_RES][k1]) && (sbr->f_table_res[HI_RES][i + 1] <= sbr->f_table_res[LO_RES][k1 + 1])) {
						break;
					}
				}
				for(k2 = 0; k2 < sbr->N_low; k2++) {
					if((sbr->f_table_res[HI_RES][i + 1] >= sbr->f_table_res[LO_RES][k2]) && (sbr->f_table_res[HI_RES][i + 2] <= sbr->f_table_res[LO_RES][k2 + 1])) {
						break;
					}
				}
			}
...
static void estimate_current_envelope(sbr_info * sbr, sbr_hfadj_info * adj, qmf_t * Xsbr, uint8_t ch)
{
	...
	for(l = 0; l < sbr->L_E[ch]; l++) {
		for(k = 0; k < sbr->N_G[l]; k++) {
			...
			for(m = sbr->f_group[l][(k << 1)]; m < sbr->f_group[l][(k << 1) + 1]; m++) {
				...
				adj->G_lim_boost[l][m - sbr->kx] = MUL(alpha, G_target) + MUL((REAL_CONST(1) - alpha), adj->G_lim_boost[l][m - sbr->kx]);
		...
		for(i = sbr->t_E[ch][l]; i < sbr->t_E[ch][l + 1]; i++) {
			...
			for(m = 0; m < sbr->M; m++) {
				...
				if(adj->S_index_mapped[m][l]) {
					...
					if(m == 0) {
						QMF_RE(Xsbr[((i + tHFAdj) << 6) + m + sbr->kx - 1]) -= (rev * MUL_R_C(MUL(adj->S_M_boost[l][0], phi_re[i_plus1]), COEF_CONST(0.00815)));
						QMF_RE(Xsbr[((i + tHFAdj) << 6) + m + sbr->kx]) -= (rev * MUL_R_C(MUL(adj->S_M_boost[l][1], phi_re[i_plus1]), COEF_CONST(0.00815)));
					}
					if((m > 0) && (m < sbr->M - 1) && (sinusoids < 16)) {
						QMF_RE(Xsbr[((i + tHFAdj) << 6) + m + sbr->kx]) -= (rev * MUL_R_C(MUL(adj->S_M_boost[l][m - 1], phi_re[i_min1]), COEF_CONST(0.00815)));
						QMF_RE(Xsbr[((i + tHFAdj) << 6) + m + sbr->kx]) -= (rev * MUL_R_C(MUL(adj->S_M_boost[l][m + 1], phi_re[i_plus1]), COEF_CONST(0.00815)));
					}
					if((m == sbr->M - 1) && (sinusoids < 16) && (m + sbr->kx + 1 < 63)) {
						QMF_RE(Xsbr[((i + tHFAdj) << 6) + m + sbr->kx]) -= (rev * MUL_R_C(MUL(adj->S_M_boost[l][m - 1], phi_re[i_min1]), COEF_CONST(0.00815)));
						QMF_RE(Xsbr[((i + tHFAdj) << 6) + m + sbr->kx + 1]) -= (rev * MUL_R_C(MUL(adj->S_M_boost[l][m + 1], phi_re[i_min1]), COEF_CONST(0.00815)));
					}
