...
**
** $Id: sbr_fbt.c,v 1.11 2003/09/21 00:00:00 menno Exp $
**/
...
uint16_t qmf_start_channel(uint8_t bs_start_freq, uint8_t bs_samplerate_mode, uint32_t sample_rate)
{
...
uint16_t qmf_stop_channel(uint8_t bs_stop_freq, uint32_t sample_rate, uint16_t k0)
{
	if(bs_stop_freq == 15) {
		return min(64, k0 * 3);
...
*/
void master_frequency_table_fs0(sbr_info * sbr, uint16_t k0, uint16_t k2, uint8_t bs_alter_scale)
{
	int8_t incr;
	uint8_t k;
	uint8_t dk;
...
/*
   version for bs_freq_scale > 0
*/
void master_frequency_table(sbr_info * sbr, uint16_t k0, uint16_t k2, uint8_t bs_freq_scale, uint8_t bs_alter_scale)
{
	...
	for(k = 0; k <= nrBand0; k++) {
		/* MAPLE */
		vDk0[k] = (int32_t) (k0 * pow((float)k1 / (float)k0, (k + 1) / (float)nrBand0) + 0.5) - (int32_t) (k0 * pow((float)k1 / (float)k0, k / (float)nrBand0) + 0.5);
	}
	...
	for(k = 0; k <= nrBand1 - 1; k++) {
		vDk1[k] = (int32_t) (k1 * pow((float)k2 / (float)k1, (k + 1) / (float)nrBand1) + 0.5) - (int32_t) (k1 * pow((float)k2 / (float)k1, k / (float)nrBand1) + 0.5);
	}
...
void derived_frequency_table(sbr_info * sbr, uint8_t bs_xover_band, uint16_t k2)
{
	uint8_t k, i;
	uint32_t minus;
