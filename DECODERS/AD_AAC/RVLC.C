...
uint8_t rvlc_scale_factor_data(ic_stream * ics, bitfile * ld)
{
	...
	ics->rev_global_gain = faad_bits_read24(ld, 8 DEBUGVAR(1, 150, "rvlc_scale_factor_data(): rev_global_gain"));

	if(ics->window_sequence == EIGHT_SHORT_SEQUENCE)
		bits = 11;

	ics->length_of_rvlc_sf = faad_bits_read24(ld, bits DEBUGVAR(1, 151, "rvlc_scale_factor_data(): length_of_rvlc_sf"));

	if(ics->noise_used) {
		ics->dpcm_noise_nrg = faad_bits_read24(ld, 9 DEBUGVAR(1, 152, "rvlc_scale_factor_data(): dpcm_noise_nrg"));
	...
	if(ics->sf_escapes_present) {
		ics->length_of_rvlc_escapes = faad_bits_read24(ld, 8 DEBUGVAR(1, 154, "rvlc_scale_factor_data(): length_of_rvlc_escapes"));
	}

	if(ics->noise_used) {
		ics->dpcm_noise_last_position = faad_bits_read24(ld, 9 DEBUGVAR(1, 155, "rvlc_scale_factor_data(): dpcm_noise_last_position"));
	}
