...
/*#ifdef __WATCOMC__
 void asm_fltround(float *);
#endif*/

static void flt_round(real_t * pf)
{
/*#ifdef __WATCOMC__
 #pragma aux asm_fltround=\
  "fld dword ptr [eax]"\
  "frndint"\
  "fstp dword ptr [eax]"\
  parm[eax] modify[];
...
void ic_prediction(ic_stream * ics, real_t * spec, pred_state * state, uint16_t frame_len)
{
	uint8_t sfb;
	uint16_t bin;
	...
	if(ics->window_sequence == EIGHT_SHORT_SEQUENCE) {
		...
		for(sfb = 0; sfb < ics->pred.limit; sfb++) {
			...
			for(bin = low; bin < high; bin++) {
				ic_predict(&state[bin], spec[bin], &spec[bin], (ics->predictor_data_present && ics->pred.prediction_used[sfb]));
			}
		}

		if(ics->predictor_data_present) {
			if(ics->pred.predictor_reset) {
				for(bin = ics->pred.predictor_reset_group_number - 1; bin < frame_len; bin += 30) {
