    ...
    if (ics->window_sequence != EIGHT_SHORT_SEQUENCE)
    {
        if (ltp->data_present)
        {
            ...
            for(i = 0; i < num_samples; i++)
            {
                ...
                ltd->x_est[i] = MUL_R_C(lt_pred_stat[num_samples + i - ltp->lag],
                    codebook[ltp->coef]);
            }

            filter_bank_ltp(fb, ics->window_sequence, win_shape, win_shape_prev,
                ltd->x_est, ltd->X_est, object_type, frame_len);

            tns_encode_frame(ics, &(ics->tns), sr_index, object_type, ltd->X_est,
                frame_len);
