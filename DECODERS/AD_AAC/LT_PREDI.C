...
**
** $Id: lt_predict.c,v 1.11 2003/09/20 00:00:00 PDSoft Exp $
**/
...
ltp_datas *lt_pred_init(unsigned int frame_len)
{
	unsigned int num_samples = frame_len << 1;

	ltp_datas *ltd = (ltp_datas *) malloc(sizeof(ltp_datas));
	if(!ltd)
		return ltd;

	ltd->x_est = (real_t *) malloc(num_samples * sizeof(real_t));
	ltd->X_est = (real_t *) malloc(num_samples * sizeof(real_t));

	return ltd;
}

void lt_pred_close(ltp_datas * ltd)
{
	if(ltd) {
		if(ltd->x_est)
			free(ltd->x_est);
...
void lt_prediction(ltp_datas * ltd, ic_stream * ics, ltp_info * ltp, real_t * spec,
				   real_t * lt_pred_stat, fb_info * fb, uint8_t win_shape, uint8_t win_shape_prev, uint8_t sr_index, uint8_t object_type, uint16_t frame_len)
{
	uint8_t sfb;
	...
	if(ics->window_sequence != EIGHT_SHORT_SEQUENCE) {
		if(ltp->data_present) {
			...
			for(i = 0; i < num_samples; i++) {
				...
				ltd->x_est[i] = MUL_R_C(lt_pred_stat[num_samples + i - ltp->lag], codebook[ltp->coef]);
			}

			filter_bank_ltp(fb, ics->window_sequence, win_shape, win_shape_prev, ltd->x_est, ltd->X_est, object_type, frame_len);

			tns_encode_frame(ics, &(ics->tns), sr_index, object_type, ltd->X_est, frame_len);
...
void lt_update_state(real_t * lt_pred_stat, real_t * time, real_t * overlap, uint16_t frame_len, uint8_t object_type)
{
	uint16_t i;
