...
**
** $Id: sbr_qmf.c,v 1.5 2003/09/20 00:00:00 PDSoft Exp $
**/
...
void sbr_qmf_analysis_32(qmfa_info * qmfa, const real_t * input, qmf_t * X, uint8_t offset)
{
	uint8_t l;
	real_t u[64];
...
void sbr_qmf_synthesis_64(qmfs_info * qmfs, const qmf_t * X, real_t * output)
{
	uint8_t l;
	int16_t n, k;
#ifdef SBR_LOW_POWER
	...
	for(l = 0; l < 32; l++) {
		...
		for(k = 0; k < 64; k++) {
			*outptr++ = MUL_R_C(qmfs->v[k], qmf_c[k]) +
				...
				MUL_R_C(qmfs->v[768 + 192 + k], qmf_c[384 + 64 + k]) + MUL_R_C(qmfs->v[1024 + k], qmf_c[512 + k]) + MUL_R_C(qmfs->v[1024 + 192 + k], qmf_c[512 + 64 + k]);
		}
	}
}
