 ...
 switch (cb){
  ...
  case 4:
        ...
        if (extra_bits){
         ...
         if ( flushbits(ld, cbtab->N) ) return -1;
         offset += (uint16_t)showbits(ld, extra_bits);
         if ( flushbits(ld, cbtab->quad_table[offset].bits - cbtab->N) ) return -1;
        } else {
         if ( flushbits(ld, cbtab->quad_table[offset].bits) ) return -1;
        }
  ...
  case 24: case 25: case 26: case 27: case 28: case 29: case 30: case 31:
        ...
        if (extra_bits)
        {
            ...
            if ( flushbits(ld, cbtab->N) ) return -1;
            offset += (uint16_t)showbits(ld, extra_bits);
            if ( flushbits(ld, cbtab->pair_table[offset].bits - cbtab->N) ) return -1;
        } else {
            if ( flushbits(ld, cbtab->pair_table[offset].bits) ) return -1;
        }
