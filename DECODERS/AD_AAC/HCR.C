...
**
** $Id: hcr.c,v 1.5 2003/09/20 00:00:00 PDSoft Exp $
**/
...
static int16_t huffman_spectral_data_2(uint8_t cb, bits_t * ld, int16_t * sp)
{
	...
	case 4:
		...
		cw = showbits(ld, cbtab->N);
		offset = cbtab->hcb_table[cw].offset;
		extra_bits = cbtab->hcb_table[cw].extra_bits;

		if(extra_bits) {
			/* we know for sure it's more than cbtab->N bits long */
			if(flushbits(ld, cbtab->N))
				return -1;
			offset += (uint16_t) showbits(ld, extra_bits);
			if(flushbits(ld, cbtab->quad_table[offset].bits - cbtab->N))
				return -1;
		} else {
			if(flushbits(ld, cbtab->quad_table[offset].bits))
				return -1;
		}

		sp[0] = cbtab->quad_table[offset].x;
		sp[1] = cbtab->quad_table[offset].y;
		sp[2] = cbtab->quad_table[offset].v;
		sp[3] = cbtab->quad_table[offset].w;
		break;
	...
	case 31:
		...
		if(extra_bits) {
			...
			if(flushbits(ld, cbtab->N))
				return -1;
			offset += (uint16_t) showbits(ld, extra_bits);
			if(flushbits(ld, cbtab->pair_table[offset].bits - cbtab->N))
				return -1;
		} else {
			if(flushbits(ld, cbtab->pair_table[offset].bits))
				return -1;
		}
		sp[0] = cbtab->pair_table[offset].x;
		sp[1] = cbtab->pair_table[offset].y;
		break;
	...
	case 9:
		...
		while(!cbtab->bin_table[offset].is_leaf) {
			uint8_t b;

			if(get1bit(ld, &b))
				return -1;
			offset += cbtab->bin_table[offset].data[b];
		}

		sp[0] = cbtab->bin_table[offset].data[0];
		sp[1] = cbtab->bin_table[offset].data[1];

		break;
	}

	/* decode sign bits */
	if(cbtab->unsigned_cb) {
...
typedef struct rsd_data_s {
	codeword_state Codewords[1024];	// FIXME max length? PCWs are not stored, so index is Codewordnr - numberOfSegments!
	bits_t Segment[512];
} rsd_data_s;

void *aacdec_hcr_init(void)
{
	return malloc(sizeof(rsd_data_s));
}

void aacdec_hcr_close(void *rsd_datas)
{
	if(rsd_datas)
		free(rsd_datas);
}
...
uint8_t reordered_spectral_data(faacDecHandle hDecoder, ic_stream * ics, bitfile * ld, int16_t * spectral_data)
{
	uint16_t g, i, presort;
	uint16_t NrCodeWords = 0, numberOfSegments = 0, BitsRead = 0;
	uint8_t numberOfSets, set;
	codeword_state *Codewords = ((rsd_data_s *) hDecoder->rsd_datas)->Codewords;
	bits_t *Segment = ((rsd_data_s *) hDecoder->rsd_datas)->Segment;
	...
	if((ics->length_of_longest_codeword == 0)
	   || (ics->length_of_reordered_spectral_data < ics->length_of_longest_codeword)
	   || (ics->max_sfb == 0)) {
			...
			for(w = 0; w < (ics->swb_offset[sfb + 1] - ics->swb_offset[sfb]); w += 4) {
				for(g = 0; g < ics->num_window_groups; g++) {
					for(i = 0; i < ics->num_sec[g]; i++) {
						...
						if(		/* process only sections that are due now */
									...
									if(!PCW_decoded) {
										...
										if((BitsRead + segmentWidth(sect_cb)) <= ics->length_of_reordered_spectral_data) {
											...
											if(segmentWidth(sect_cb) > 32) {
												...
												faad_bits_inline_flush(ld, segmentWidth(sect_cb) - 32);
												Segment[numberOfSegments].bufa = faad_showbits(ld, 32), faad_bits_inline_flush(ld, 32);
											} else {
												Segment[numberOfSegments].bufa = faad_showbits(ld, segmentWidth(sect_cb));
												Segment[numberOfSegments].bufb = 0;
												faad_bits_inline_flush(ld, segmentWidth(sect_cb));
											}
											...
											if(BitsRead < ics->length_of_reordered_spectral_data) {
												...
												if(additional_bits > 32) {
													...
													faad_bits_inline_flush(ld, additional_bits - 32);
													lw = faad_showbits(ld, 32);
													faad_bits_inline_flush(ld, 32);
												} else {
													lw = faad_showbits(ld, additional_bits);
													hw = 0;
													faad_bits_inline_flush(ld, additional_bits);
												}
												rewind_lword(&hw, &lw, additional_bits + Segment[numberOfSegments - 1].len);
												if(Segment[numberOfSegments - 1].len > 32) {
													Segment[numberOfSegments - 1].bufb = hw + showbits(&Segment[numberOfSegments - 1], Segment[numberOfSegments - 1].len - 32);
													Segment[numberOfSegments - 1].bufa = lw + showbits(&Segment[numberOfSegments - 1], 32);
												} else {
													Segment[numberOfSegments - 1].bufa = lw + showbits(&Segment[numberOfSegments - 1], Segment[numberOfSegments - 1].len);
	...
	for(set = 1; set <= numberOfSets; set++) {
		...
		for(trial = 0; trial < numberOfSegments; trial++) {
			...
			for(codewordBase = 0; codewordBase < numberOfSegments; codewordBase++) {
				...
				if(!Codewords[codeword_index].decoded) {
					if(Segment[segment_index].len > 0) {
						...
						if(huffman_spectral_data_2(Codewords[codeword_index].cb, &Segment[segment_index], &spectral_data[Codewords[codeword_index].sp_offset]) >= 0) {
			...
			if(set_decoded == 0)
				break;			/* no undecoded codewords left in this set */
