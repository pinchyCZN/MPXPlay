...
int a52_frame(a52_state_t * state, uint8_t * buf, int *flags, level_t * level, sample_t bias, int frame_size)
{
	...
	if(a52_crc16_block(buf + 2, frame_size - 2) != 0)	// check crc of frame without syncword
		return 1;				// failed
	...
	state->output = a52_downmix_init(acmod, *flags, level, state->clev, state->slev);
	if(state->output < 0)
		return 1;
	...
	*flags = state->output;
	...
	state->ba[0].deltbae = state->ba[1].deltbae = state->ba[2].deltbae = state->ba[3].deltbae = state->ba[4].deltbae = DELTA_BIT_NONE;
...
static int parse_exponents(a52_state_t * state, int expstr, int ngrps, uint8_t exponent, uint8_t * dest)
{
	int exps;
...
static inline int zero_snr_offsets(int nfchans, a52_state_t * state)
{
	...
	if((state->csnroffst) || (state->chincpl && state->cplba.bai >> 3) ||	/* cplinu, fsnroffst */
...
static void coeff_get(a52_state_t * state, sample_t * coeff, expbap_t * expbap, quantizer_set_t * quant, level_t level, int dither, int end)
{
	int i;
	uint8_t *exp;
	int8_t *bap;
	...
	for(i = 0; i < end; i++) {
		...
		case -1:
			if(quant->q1_ptr >= 0) {
				COEFF(coeff[i], quant->q1[quant->q1_ptr--], level, factor, exp[i]);
				continue;
			} else {
				int code;
		...
		case -2:
			if(quant->q2_ptr >= 0) {
				COEFF(coeff[i], quant->q2[quant->q2_ptr--], level, factor, exp[i]);
				continue;
			} else {
				int code;
		...
		case 3:
			COEFF(coeff[i], q_3[bitstream_get(state, 3)], level, factor, exp[i]);
			continue;
		...
		case 4:
			COEFF(coeff[i], q_5[bitstream_get(state, 4)], level, factor, exp[i]);
			continue;

		default:
			COEFF(coeff[i], bitstream_get_2(state, bapi) << (16 - bapi), level, factor, exp[i]);
		}
	}
}

static void coeff_get_coupling(a52_state_t * state, int nfchans, level_t * coeff, sample_t(*samples)[256], quantizer_set_t * quant, uint8_t dithflag[5])
{
			...
			case 0:
				for(ch = 0; ch < nfchans; ch++)
					if((state->chincpl >> ch) & 1) {
						if(dithflag[ch])
#ifndef LIBA52_FIXED
							samples[ch][i] = (scale_factor[exp[i]] * cplco[ch] * dither_gen(state));
#else
							COEFF(samples[ch][i], dither_gen(state), cplco[ch], scale_factor, exp[i]);
#endif
						else
							samples[ch][i] = 0;
					}
...
#else
					COEFF(samples[ch][i], cplcoeff, cplco[ch], scale_factor, exp[i]);
#endif
			i++;
		}
	}
...
int a52_block(a52_state_t * state)
{
	...
	do {
		if(bitstream_get(state, 1)) {	/* dynrnge */
			...
			if(state->dynrnge) {
				...
				range = ((((dynrng & 0x1f) | 0x20) << 13) * scale_factor[3 - (dynrng >> 5)]);
#else
...
#ifndef LIBA52_FIXED
						state->cplco[i][j] = cplcomant * scale_factor[cplcoexp + mstrcplco];
#else
	...
	if(cplexpstr != EXP_REUSE) {
		...
		if(parse_exponents(state, cplexpstr, ncplgrps, cplabsexp, state->cpl_expbap.exp + state->cplstrtmant))
			return 1;
	}
	for(i = 0; i < nfchans; i++)
		if(chexpstr[i] != EXP_REUSE) {
			...
			if(parse_exponents(state, chexpstr[i], nchgrps, state->fbw_expbap[i].exp[0], state->fbw_expbap[i].exp + 1))
				return 1;
	...
	if(lfeexpstr != EXP_REUSE) {
		...
		if(parse_exponents(state, lfeexpstr, 2, state->lfe_expbap.exp[0], state->lfe_expbap.exp + 1))
			return 1;
	}
	...
	if(bitstream_get(state, 1)) {	/* deltbaie */
		...
		if(state->chincpl &&	/* cplinu */
		   (state->cplba.deltbae == DELTA_BIT_NEW) && parse_deltba(state, state->cplba.deltba))
			return 1;
		for(i = 0; i < nfchans; i++)
			if((state->ba[i].deltbae == DELTA_BIT_NEW) && parse_deltba(state, state->ba[i].deltba))
				return 1;
	}
	...
	for(i = 0; i < nfchans; i++) {
		...
		coeff_get(state, samples + 256 * i, state->fbw_expbap + i, &quant, coeff[i], dithflag[i], state->endmant[i]);

		if((state->chincpl >> i) & 1) {
			if(!done_cpl) {
				done_cpl = 1;
				coeff_get_coupling(state, nfchans, coeff, (sample_t(*)[256]) samples, &quant, dithflag);
			}
			j = state->cplendmant;
		} else
			j = state->endmant[i];
	...
	if(state->acmod == 2) {
		...
		end = ((state->endmant[0] < state->endmant[1]) ? state->endmant[0] : state->endmant[1]);
	...
	if(state->lfeon) {
		if(state->output & A52_LFE) {
			coeff_get(state, samples - 256, &state->lfe_expbap, &quant, state->dynrng, 0, 7);
			for(i = 7; i < 256; i++)
				(samples - 256)[i] = 0;
			a52_imdct_512(samples - 256, samples + 1536 - 256, state->bias);
		} else {
			/* just skip the LFE coefficients */
			coeff_get(state, samples + 1280, &state->lfe_expbap, &quant, 0, 0, 7);
		}
	}
...
void a52_reset(a52_state_t * state)
{
	memset(state->samples, 0, (256 * 12 * sizeof(sample_t)));
}
