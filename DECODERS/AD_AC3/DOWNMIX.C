...
int a52_downmix_init(int input, int flags, level_t * level, level_t clev, level_t slev)
{
	...
	if(output == A52_STEREO && (input == A52_DOLBY || (input == A52_3F && clev == LEVEL(LEVEL_3DB))))
		output = A52_DOLBY;
...
static void mix4to1(sample_t * samples, sample_t bias)
{
	...
	for(i = 0; i < 256; i++)
		samples[i] += BIAS(samples[i + 256] + samples[i + 512] + samples[i + 768]);
}
...
static void mix5to1(sample_t * samples, sample_t bias)
{
	...
	for(i = 0; i < 256; i++)
		samples[i] += BIAS(samples[i + 256] + samples[i + 512] + samples[i + 768] + samples[i + 1024]);
}
