...
int unpack_init(WavpackContext * wpc)
{
	...
	if(wps->wphdr.block_samples && (wps->wphdr.block_index != DATA_INVALID))
	...
	if(wps->wphdr.block_samples) {
		...
		if((wps->wphdr.flags & FLOAT_DATA) && wps->float_flags & (FLOAT_EXCEPTIONS | FLOAT_ZEROS_SENT | FLOAT_SHIFT_SENT | FLOAT_SHIFT_SAME))
			wpc->lossy_blocks = TRUE;
	}
...
int init_wv_bitstream(WavpackContext * wpc, WavpackMetadata * wpmd)
{
	...
	if(wpmd->data)
		bs_open_read(&wps->wvbits, wpmd->data, (uchar *) wpmd->data + wpmd->byte_length, NULL, NULL, 0);
	else if(wpmd->byte_length)
		bs_open_read(&wps->wvbits, wpc->read_buffer, wpc->read_buffer + sizeof(wpc->read_buffer), wpc->infile, wpc->streamhand, wpmd->byte_length + (wpmd->byte_length & 1));
...
int read_decorr_samples(WavpackStream * wps, WavpackMetadata * wpmd)
{
	...
	while(dpp-- > wps->decorr_passes && byteptr < endptr)
		if(dpp->term > MAX_TERM) {
			dpp->samples_A[0] = exp2s((short)(byteptr[0] + ((short)byteptr[1] << 8)));
			dpp->samples_A[1] = exp2s((short)(byteptr[2] + ((short)byteptr[3] << 8)));
			byteptr += 4;

			if(!(wps->wphdr.flags & MONO_FLAG)) {
				dpp->samples_B[0] = exp2s((short)(byteptr[0] + ((short)byteptr[1] << 8)));
				dpp->samples_B[1] = exp2s((short)(byteptr[2] + ((short)byteptr[3] << 8)));
				byteptr += 4;
			}
		} else if(dpp->term < 0) {
			dpp->samples_A[0] = exp2s((short)(byteptr[0] + ((short)byteptr[1] << 8)));
			dpp->samples_B[0] = exp2s((short)(byteptr[2] + ((short)byteptr[3] << 8)));
			byteptr += 4;
		} else {
			int m = 0, cnt = dpp->term;

			while(cnt--) {
				dpp->samples_A[m] = exp2s((short)(byteptr[0] + ((short)byteptr[1] << 8)));
				byteptr += 2;

				if(!(wps->wphdr.flags & MONO_FLAG)) {
					dpp->samples_B[m] = exp2s((short)(byteptr[0] + ((short)byteptr[1] << 8)));
					byteptr += 2;
				}
...
long unpack_samples(WavpackContext * wpc, long *buffer, ulong sample_count)
{
	...
	long mute_limit = (1UL << ((flags & MAG_MASK) >> MAG_LSB)) + 2;
	struct decorr_pass *dpp;
	long read_word, *bptr;
	int tcount, m = 0;
	...
	//////////////////// handle version 4 stereo data ////////////////////////
		...
		for(bptr = buffer, i = 0; i < sample_count; ++i) {
			...
			if((left = get_word(wps, 0)) == WORD_EOF || (right = get_word(wps, 1)) == WORD_EOF)
				break;
	...
	if(flags & FLOAT_DATA)
		float_normalize(buffer, (flags & MONO_FLAG) ? i : i * 2, 127 - wps->float_norm_exp + wpc->norm_offset);
