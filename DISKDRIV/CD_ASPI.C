...
static unsigned int cdaspi_get_media_status(struct winaspi_drive_info_s *wdis)
{
	...
	case 0:
		status = CDASPI_CDMEDIA_PRESENT;
		break;
	case 2:
		if(wdis->cd_statusinfo.asc == 0x3A) {
			switch (wdis->cd_statusinfo.ascq) {
			case 0x00:
				status = CDASPI_CDMEDIA_NOT_PRESENT;
				break;
			case 0x01:
				status = CDASPI_CDMEDIA_NOT_PRESENT_TRAY_CLOSED;
				break;
			case 0x02:
				status = CDASPI_CDMEDIA_NOT_PRESENT_TRAY_OPEN;
				break;
			}
		}
	}
