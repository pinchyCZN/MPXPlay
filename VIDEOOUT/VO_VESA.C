...
static unsigned int videoout_vesa_init(struct mpxplay_videoout_info_s *voi)
{
 ...
 for(i=0;i<vds->vesa_modes;i++)
  if(loc_vesa_querymode(vds,vds->vesa_mode_list[i],&svgainfos))
   fprintf(stdout,"%2d. %4.4Xh %4dx%4dx%2d attr:%8.8X pages:%2d phys_addr:%8.8X\n",i+1,vds->vesa_mode_list[i],svgainfos.XResolution,svgainfos.YResolution,svgainfos.BitsPerPixel,svgainfos.ModeAttributes,(long)svgainfos.NumberOfImagePages,svgainfos.PhysBasePtr);
#endif
 ...
 if(!voi->config_res_x){
  ...
  for(i=0;i<vds->vesa_modes;i++){
   if(loc_vesa_querymode(vds,vds->vesa_mode_list[i],&svgainfos)){
    if((svgainfos.ModeAttributes&VESAATTR_IS_GFX_MODE) && (highestx<svgainfos.XResolution) && (svgainfos.BitsPerPixel==voi->config_bpp) && (svgainfos.ModeAttributes&VESAATTR_LFB_SUPPORTED)){
