...
{
 switch(button){
  case MOUSE_BUTTON_LEFT  :if(MouseFuncBtn1Pri || MouseFuncBtn1Sec)
                            return 1;
                           break;
  case MOUSE_BUTTON_CENTER:if(MouseFuncBtn2Pri || MouseFuncBtn2Sec)
                            return 1;
                           break;
  case MOUSE_BUTTON_RIGHT :if(MouseFuncBtn3Pri || MouseFuncBtn3Sec)
                            return 1;
                           break;
 }
 return 0;
  ...
  if(cbp->mouse_ax&MOUSE_EVENT_PRESS){
   ...
   if(cbp->mouse_ax&MOUSE_EVENT_RELEASE){
    ...
    if(cbp->buttonpresstime_begin){
     ...
     if((timelen>=MOUSE_CLICK_TIME) && (timelen<MOUSE_PRESS_TIME) && !cbp->mouse_event
       && mouse_check_func(cbp->readbutton)){
      cbp->mousebutton=cbp->readbutton;
      cbp->clicktype=MOUSE_CLICKTYPE_SINGLE;
     }
  ...
  switch(inprec[n].EventType){
   case KEY_EVENT:
    if(inprec[n].Event.KeyEvent.bKeyDown){
     unsigned int scancode=newfunc_keyboard_winkey_to_extkey(inprec[n].Event.KeyEvent.dwControlKeyState,inprec[n].Event.KeyEvent.wVirtualKeyCode,inprec[n].Event.KeyEvent.uChar.AsciiChar);
     if(scancode)
