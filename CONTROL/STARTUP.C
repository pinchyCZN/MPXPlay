...
void mpxplay_control_startup_loadlastlist(void)
{
	...
	if(su_startuptype & STARTUP_FLAG_LOAD) {
		...
		for(i = loadside_begin; i <= loadside_end; i++, psi++) {
			if(!su_oldlistname[i][0])
				continue;
			if((su_startuptype & (STARTUP_FLAG_RESDIR | STARTUP_FLAG_2SIDES))
			   && ((psi != mvp->psil) || (!(su_startuptype & STARTUP_FLAG_CMOS) && !(playlistload & PLL_TYPE_LOAD) && !playlist_loadsub_getinputfile(psi) && !drivescanletters))) {
	...
	if((su_startuptype & STARTUP_FLAG_CMOS) && (su_startuptype & STARTUP_FLAG_SAVE) && !(mvp->aui->card_handler->infobits & SNDCARD_IGNORE_STARTUP)) {
		...
		mpxplay_timer_addfunc(&mpxplay_control_startup_savetocmos, NULL, MPXPLAY_TIMERTYPE_SIGNAL | MPXPLAY_TIMERFLAG_MULTIPLY | MPXPLAY_TIMERTYPE_REPEAT | MPXPLAY_TIMERFLAG_MVPDATA,
							  MPXPLAY_SIGNALTYPE_NEWFILE);
	}
}
...
void mpxplay_control_startup_getstartpos(mainvars * mvp)
{
	...
	if((su_startuptype & STARTUP_FLAG_LOAD) && !(mvp->aui->card_handler->infobits & SNDCARD_IGNORE_STARTUP)) {
		if(su_startuptype & STARTUP_FLAG_CMOS) {
			...
			if(!playstartsong
			   && (((su_oldlisttype[su_playside] == mvp->psip->editloadtype) && (!s || pds_stricmp(&su_oldlistname[su_playside][0], fullname) == 0)) || (pds_stricmp(su_oldsongname, fullname) == 0))) {
