 ...
 do{
  ...
  if( extkey==kb[57].c                      // f
   ...
   switch(cfi->crossfadetype&(CFT_FADEOUT|CFT_FADEIN)){
    case  0                      :cft=CFT_FADEOUT;           break;
    case  CFT_FADEOUT            :cft=CFT_FADEOUT|CFT_FADEIN;break;
    case  CFT_FADEIN             :cft=0;                     break;
    case (CFT_FADEOUT|CFT_FADEIN):cft=CFT_FADEIN;            break;
   }
  ...
  if( extkey==kb[59].c                      // r
   || extkey==kb[60].c){                    // R
   switch(playreplay){
    case 0          :playreplay=REPLAY_SONG;       // 0->1
                     mvp->newfilenum=NULL;  // !!! ???
                     funcbit_enable(selected,RDT_EDITOR);
                     break;
    case REPLAY_SONG:playreplay=REPLAY_LIST;       // 1->2
                     mvp->foundfile=1;break;
    default         :playreplay=0;                 // 2->0
  ...
  if( extkey==kb[133].c                     // e
   ...
   if(funcbit_test(desktopmode,DTM_EDIT_ALLFILES)){
    ...
    if(funcbit_test(desktopmod_mpxini,DTM_EDIT_ALLFILES)){
     ...
     desktopmode=(desktopmode&(~(DTM_EDIT_SONGNUM|DTM_EDIT_SONGTIME|DTM_EDIT_FILENAMES|DTM_EDIT_SHOWDIRDATE)))|(desktopmod_mpxini&(DTM_EDIT_SONGNUM|DTM_EDIT_SONGTIME|DTM_EDIT_FILENAMES|DTM_EDIT_SHOWDIRDATE));
 ...
 do{
  ...
  if( extkey==kb[145].c){                   // ctrl-'\'
   if((psie->editsidetype&PLT_DIRECTORY) && !psie->sublistlevel)
    ...
    if(!(psie->editsidetype&PLT_DOOMQUEUE))
...
static void mpxplay_control_keyboard_id3search_check(unsigned int extkey,struct mainvars *mvp)
{
 ...
 if(s_init){
  switch(extkey){
   ...
   case KEY_TAB       :
   case KEY_ENTER1    :
   case KEY_ENTER2    :pds_pushkey(extkey); // push back the keycode for the keyboard_primary (start song at enter)
                       goto mckic_close;
   case KEY_ESC       :
   ...
   case KEY_BACKSPACE :if(ss_len)
			searchstring[ss_len-1]=0;
		       extkey=search_step=0;
		       break;
   case KEY_UP_GRAY   :
   case KEY_UP_WHITE  :if(psi->editorhighline>psi->firstentry){
			...
			search_step=0;
		       ...
		       break;
   case KEY_DOWN_GRAY :
   case KEY_DOWN_WHITE:if(psi->editorhighline<psi->lastentry)
			search_begin=psi->editorhighline+1;
		       else
			search_step=0;
		       extkey=0;
