		...
		if(linep->varnamep) {
			if(pds_strlicmp(linep->varnamep, "HandlerCfg") == 0) {
				...
				if(nextdata) {
					...
					} else if(pds_strlicmp(currdata, "LPT") == 0) {
				...
				current_funccount = 0;
#ifdef MPX_DEBUG_SERIAL
				sprintf(sout, "hand:%s port:%d baud:%d datalen:%d ", handrp->handname, current_handlerdatas->portselect, current_handlerdatas->portdatas.baud, current_handlerdatas->indatalen);
			...
			if(handp->handlerdatas) {
				if(!handp->handlerroutines->initport || handp->handlerroutines->initport(handp->handlerdatas)) {
					...
					snprintf(sout, sizeof(sout), "Warning: Couldn't initialize port control (%s on %s%d)!",
							 ...
							 (handp->handlerdatas->portselect <= 4) ? "COM" : "LPT", (handp->handlerdatas->portselect <= 4) ? handp->handlerdatas->portselect : (handp->handlerdatas->portselect - 4));
					display_warning_message(sout);
...
static unsigned int serial_com_port_init(struct serial_port_data_s *portdatas, unsigned int portselect, unsigned int useirq)
{
	...
	portdatas->comm_port_handle = CreateFile(pcCommPort, GENERIC_READ | GENERIC_WRITE, 0,	// must be opened with exclusive-access
...
static void __interrupt __loadds serial_handler_comcntrl_irqroutine(void)
{
	...
	if((++handdatas->inbytecount) >= handdatas->indatalen) {
		if(handdatas->flags & SERIAL_HANDDATAFLAG_DIRECTKEY) {
			...
			lircStr("UIR\n",
					(handdatas->
					 inbytesstore[0] << 24) | (((handdatas->indatalen > 1) ? handdatas->inbytesstore[1] : 0) << 16) | (((handdatas->indatalen > 2) ? handdatas->
																														inbytesstore[2] : 0) << 8) | ((handdatas->indatalen >
																																					   3) ? handdatas->inbytesstore[3] : 0), 0);
#endif
		}
	}
	outp(UART_IER, 0x01);		// enable interrupts for data ready on the serial line (8250)
	...
	if(necdatas->use_lpt) {
		...
		if(necdatas->lpt_butt) {
			...
			for(i = 0; i < handdatas->number_of_funcs; i++) {
				...
				if(funcp->databytes[0])
					found = 0;
				if(funcp->databytes[1])
					found = 0;
				if(funcp->databytes[2] != (necdatas->lpt_butt >> 3))
					found = 0;
				if(funcp->databytes[3] != ((necdatas->lpt_butt & 0x07) >> 2))
					found = 0;
				if(funcp->databytes[4] != ((necdatas->lpt_butt & 0x03) >> 1))
					found = 0;
				if(funcp->databytes[5] != (necdatas->lpt_butt & 0x01))
					found = 0;
				if(found) {
					pds_pushkey(funcp->keycode);
#ifdef MPX_DEBUG_SERIAL
			...
			if(necdatas->lpt_butt & 0x01)
				pds_textdisplay_printf("0");
			if(necdatas->lpt_butt & 0x02)
				pds_textdisplay_printf("1");
			if(necdatas->lpt_butt & 0x04)
				pds_textdisplay_printf("2");
			if(necdatas->lpt_butt & 0x08)
				pds_textdisplay_printf("3");
#endif
		}
	} else
		necdatas->lpt_butt = 0;
		...
		if(necdatas->tscdif <= (cmdperiod * irupscale) / 1000) {
			if(necdatas->pulse == 0xff) {
				if((necdatas->tscdif <= (irdownscale * firstperiod) / 1000) || (necdatas->tscdif > (irupscale * firstperiod) / 1000)) {
					...
					necdatas->pulse = 0;
			} else if(necdatas->pulse < 0x20) {
				if(necdatas->tscdif <= (bitperiod * irdownscale) / 1000) {
					necdatas->pulse = 0xff;
				} else if(necdatas->tscdif > (2 * bitperiod * irupscale) / 1000) {
					...
					if(necdatas->pulse == 0x20) {
						...
						for(i = 0; i < handdatas->number_of_funcs; i++) {
							unsigned int found = 1;
							if(funcp->databytes[0] != ((necdatas->ircode & 0x0000ff00) >> 8))
								found = 0;
							if(funcp->databytes[1] != ((necdatas->ircode & 0x000000ff)))
								found = 0;
							if(funcp->databytes[2] != ((necdatas->ircode & 0xff000000) >> 24))
								found = 0;
							if(funcp->databytes[3] != ((necdatas->ircode & 0x00ff0000) >> 16))
								found = 0;
							if(found) {
...
static void ConfigDataSocket(ftpserv_communication_s * fcs, char *data)
{
	...
	nr = sscanf(data, "%u,%u,%u,%u,%u,%u", &(ip[0]), &(ip[1]), &(ip[2]), &(ip[3]), &pHi, &pLo);
	if(nr != 6) {
		...
		if(longFormat) {
			...
			if(year_file == year_curr) {
				snprintf(line, sizeof(line), "-rw-rw-rw-   1 user     ftp  %11ld %s %2.2d %2.2d:%2.2d %s\r\n",
						 entry.size, strMon[entry.fdate.month - 1], (unsigned int)entry.fdate.day, (unsigned int)entry.fdate.hours, (unsigned int)entry.fdate.minutes, entry.name);
			} else {
				snprintf(line, sizeof(line), "-rw-rw-rw-   1 user     ftp  %11ld %s %2.2d %5d %s\r\n", entry.size, strMon[entry.fdate.month - 1], (unsigned int)entry.fdate.day, year_file, entry.name);
			}
