...
static void CMI8X38_card_info(struct mpxplay_audioout_info_s *aui)
{
	...
	char sout[100];
	sprintf(sout, "CMI : %s soundcard found on port:%4.4X irq:%d chipver:%d max-chans:%d", card->pci_dev->device_name, card->iobase, card->irq, card->chip_version, card->max_channels);
...
static int CMI8X38_adetect(struct mpxplay_audioout_info_s *aui)
{
	...
	card = (struct cmi8x38_card *)calloc(1, sizeof(struct cmi8x38_card));
	if(!card)
		return 0;
