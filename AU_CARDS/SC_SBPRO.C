...
{
	char sout[100];
	sprintf(sout, "SBP : SB-pro card found : SET BLASTER=A%3X I%d D%d T%d", aui->card_port, aui->card_irq, aui->card_isa_dma, aui->card_type);
...
static void SB_select_cardroutines(struct mpxplay_audioout_info_s *aui)
{
	switch (aui->card_type) {
	case SB_CARDTYPE_10:
		SB_sndcard_info.irq_routine = SB1_irq_routine;
		break;
	case SB_CARDTYPE_200:
	case SB_CARDTYPE_201:
	case SB_CARDTYPE_PRO:
		SB_sndcard_info.irq_routine = SB2pro_irq_routine;
		break;
	}
	switch (aui->card_type) {
	case SB_CARDTYPE_10:
		break;					// no mixer on 1.x
	case SB_CARDTYPE_200:
	case SB_CARDTYPE_201:
		SB_sndcard_info.card_mixerchans = &sb20_mixer_info;
		break;
	case SB_CARDTYPE_PRO:
		SB_sndcard_info.card_mixerchans = &sbpro_mixer_info;
		break;
	}
}
