...
static void snd_emu20kx_set_output_format(struct emu20kx_card_s *card, struct mpxplay_audioout_info_s *aui)
{
	...
	case MPXPLAY_WAVEID_PCM_FLOAT:
		card->sfnum = SRC_SF_F32;
		break;
	default:
		switch (aui->bits_card) {
		case 32:
			card->sfnum = SRC_SF_S32;
			break;
		case 24:
			card->sfnum = SRC_SF_S24;
			break;
		default:
			card->sfnum = SRC_SF_S16;
			aui->bits_card = 16;
			break;
		}
...
static void EMU20KX_card_info(struct mpxplay_audioout_info_s *aui)
{
	...
	char sout[100];
	sprintf(sout, "XFI : Creative %s (%4.4X) found on port:%4.4X irq:%d", card->pci_dev->device_name, card->subsys_id, card->iobase, card->irq);
...
static int EMU20KX_adetect(struct mpxplay_audioout_info_s *aui)
{
	...
	mpxplay_debugf(XFI_DEBUG_OUTPUT, "vend_id:%4.4X dev_id:%4.4X subid:%8.8X port:%8.8X", card->pci_dev->vendor_id, card->pci_dev->device_id, card->subsys_id, card->iobase);
