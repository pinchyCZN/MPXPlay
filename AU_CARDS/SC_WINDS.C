...
static mpxp_uint8_t winds_channel_matrix[MPXP_WINDSOUND_MAX_CHANNELS - 2][MPXP_WINDSOUND_MAX_CHANNELS] = {
	{MPXPLAY_PCMOUTCHAN_FRONT_LEFT, MPXPLAY_PCMOUTCHAN_FRONT_RIGHT, MPXPLAY_PCMOUTCHAN_LFE, MPXPLAY_PCMOUTCHAN_DISABLED, MPXPLAY_PCMOUTCHAN_DISABLED, MPXPLAY_PCMOUTCHAN_DISABLED,
	 MPXPLAY_PCMOUTCHAN_DISABLED, MPXPLAY_PCMOUTCHAN_DISABLED},
	{MPXPLAY_PCMOUTCHAN_FRONT_LEFT, MPXPLAY_PCMOUTCHAN_FRONT_RIGHT, MPXPLAY_PCMOUTCHAN_REAR_LEFT, MPXPLAY_PCMOUTCHAN_REAR_RIGHT, MPXPLAY_PCMOUTCHAN_DISABLED, MPXPLAY_PCMOUTCHAN_DISABLED,
	 MPXPLAY_PCMOUTCHAN_DISABLED, MPXPLAY_PCMOUTCHAN_DISABLED},
	{MPXPLAY_PCMOUTCHAN_FRONT_LEFT, MPXPLAY_PCMOUTCHAN_FRONT_RIGHT, MPXPLAY_PCMOUTCHAN_FRONT_CENTER, MPXPLAY_PCMOUTCHAN_REAR_LEFT, MPXPLAY_PCMOUTCHAN_REAR_RIGHT, MPXPLAY_PCMOUTCHAN_DISABLED,
	 MPXPLAY_PCMOUTCHAN_DISABLED, MPXPLAY_PCMOUTCHAN_DISABLED},
	{MPXPLAY_PCMOUTCHAN_FRONT_LEFT, MPXPLAY_PCMOUTCHAN_FRONT_RIGHT, MPXPLAY_PCMOUTCHAN_FRONT_CENTER, MPXPLAY_PCMOUTCHAN_LFE, MPXPLAY_PCMOUTCHAN_REAR_LEFT, MPXPLAY_PCMOUTCHAN_REAR_RIGHT,
	 MPXPLAY_PCMOUTCHAN_DISABLED, MPXPLAY_PCMOUTCHAN_DISABLED},
	{MPXPLAY_PCMOUTCHAN_FRONT_LEFT, MPXPLAY_PCMOUTCHAN_FRONT_RIGHT, MPXPLAY_PCMOUTCHAN_FRONT_CENTER, MPXPLAY_PCMOUTCHAN_REAR_LEFT, MPXPLAY_PCMOUTCHAN_REAR_RIGHT, MPXPLAY_PCMOUTCHAN_SIDE_LEFT,
	 MPXPLAY_PCMOUTCHAN_SIDE_RIGHT, MPXPLAY_PCMOUTCHAN_DISABLED},
	{MPXPLAY_PCMOUTCHAN_FRONT_LEFT, MPXPLAY_PCMOUTCHAN_FRONT_RIGHT, MPXPLAY_PCMOUTCHAN_FRONT_CENTER, MPXPLAY_PCMOUTCHAN_LFE, MPXPLAY_PCMOUTCHAN_REAR_LEFT, MPXPLAY_PCMOUTCHAN_REAR_RIGHT,
	 MPXPLAY_PCMOUTCHAN_SIDE_LEFT, MPXPLAY_PCMOUTCHAN_SIDE_RIGHT}
};
...
extern unsigned int aucards_select_devicenum;
extern int aucards_select_config;
...
static struct pds_win32dllcallfunc_t scwinds_kernel32_funcs[] = {
	{"GetConsoleWindow", NULL, 0},	// returns HWND
	{NULL, NULL, 0}
};

static HMODULE scwinds_kernel32_dllhand;

static HWND scwinds_getconsolewindow(void)
{
	HWND console_window;

	if(!scwinds_kernel32_dllhand) {
		scwinds_kernel32_dllhand = newfunc_dllload_winlib_load("kernel32.dll");
		if(!scwinds_kernel32_dllhand)
			goto err_out_gcw;
		if(!newfunc_dllload_winlib_getfuncs(scwinds_kernel32_dllhand, scwinds_kernel32_funcs))
			goto err_out_gcw;
	}
	console_window = (HWND) newfunc_dllload_winlib_callfunc(&scwinds_kernel32_funcs[0], NULL, NULL, NULL);
	if(console_window)
...
static int WINDS_detect(struct mpxplay_audioout_info_s *aui)
{
	...
	if(aucards_select_config >= 0)
		card->config_select = aucards_select_config;
	else
		card->config_select = AUCARDSCONFIG_WDS_DEFAULT;

	if(aucards_select_devicenum) {
		card->devicenum_select = aucards_select_devicenum;
	...
	if(IDirectSound_SetCooperativeLevel(card->hds, scwinds_getconsolewindow(), DSSCL_EXCLUSIVE))
		goto err_out_detect;
	...
	if(aui->chan_card > 2)
		aucards_card_channelmap = &winds_channel_matrix[aui->chan_card - 3][0];
...
static void WINDS_close(struct mpxplay_audioout_info_s *aui)
{
	...
	if(scwinds_kernel32_dllhand) {
		newfunc_dllload_winlib_close(scwinds_kernel32_dllhand);
		scwinds_kernel32_dllhand = NULL;
	}
}
...
static void WINDS_setrate(struct mpxplay_audioout_info_s *aui)
{
	...
	aui->chan_card = (aui->chan_set) ? aui->chan_set : 2;
	if(aui->chan_card < 2)		// mono files converted to stereo in au_mixer
		aui->chan_card = 2;
	if(aui->chan_card > MPXP_WINDSOUND_MAX_CHANNELS)
		aui->chan_card = MPXP_WINDSOUND_MAX_CHANNELS;
