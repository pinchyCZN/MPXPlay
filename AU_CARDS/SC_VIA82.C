...
static void VIA82XX_card_info(struct mpxplay_audioout_info_s *aui)
{
	...
	char sout[100];
	sprintf(sout, "VIA : %s soundcard found on port:%4.4X irq:%d chiprev:%2.2X model:%4.4X", card->pci_dev->device_name, card->iobase, card->irq, card->chiprev, card->model);
...
static void VIA82XX_setrate(struct mpxplay_audioout_info_s *aui)
{
	...
	case 32000:
		spdif_rate = AC97_SC_SPSR_32K;
		break;
	case 44100:
		spdif_rate = AC97_SC_SPSR_44K;
		break;
	default:
		spdif_rate = AC97_SC_SPSR_48K;
		break;
	}
	...
	if(card->pci_dev->device_id == PCI_DEVICE_ID_VT82C686) {
		outb(card->iobase + VIA_REG_OFFSET_TYPE, VIA_REG_TYPE_AUTOSTART | VIA_REG_TYPE_16BIT | VIA_REG_TYPE_STEREO);	// old: 0xB0
