...
{
	if((card->chips & EMU_CHIPS_0106)
	   && ((card->serial == 0x10061102) || (card->serial == 0x10071102) || (card->serial == 0x10091462) || (card->serial == 0x30381297) || (card->serial == 0x10121102)
		   || !card->card_capabilities->subsystem)) {
...
static void snd_ca0106_hw_init(struct emu10k1_card *card)
{
	...
	snd_ca0106_ptr_write(card, SPCS0, 0,
						 ...
						 SPCS_CHANNELNUM_LEFT | SPCS_SOURCENUM_UNSPEC | SPCS_GENERATIONSTATUS | 0x00001200 | 0x00000000 | SPCS_EMPHASIS_NONE | SPCS_COPYRIGHT);
