...
static void snd_es1371_dac1_rate(struct ensoniq_card_s *card, unsigned int rate)
{
	...
	outl((card->port + ES_REG_1371_SMPRATE), r);
	snd_es1371_src_write(card, ES_SMPREG_DAC1 + ES_SMPREG_INT_REGS, (snd_es1371_src_read(card, ES_SMPREG_DAC1 + ES_SMPREG_INT_REGS) & 0x00ff) | ((freq >> 5) & 0xfc00));
...
static void ES1371_card_info(struct mpxplay_audioout_info_s *aui)
{
	...
	char sout[100];
	sprintf(sout, "ENS : Ensoniq %s found on port:%4.4X irq:%d rev:%2.2X", card->pci_dev->device_name, card->port, card->irq, card->chiprev);
