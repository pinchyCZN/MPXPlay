...
{
	char sout[100];
	sprintf(sout, "ESS : soundcard found (type=%4.4X) : SET BLASTER=A%3X I%d D%d H%d", aui->card_type, aui->card_port, aui->card_irq, aui->card_isa_dma, aui->card_isa_hidma);
...
static aucards_onemixerchan_s ess18xx_master_switch =
	{ AU_MIXCHANFUNCS_PACK(AU_MIXCHAN_MASTER, AU_MIXCHANFUNC_MUTE), 2, {{0x60, 1, 6, SUBMIXCH_INFOBIT_REVERSEDVALUE}, {0x62, 1, 6, SUBMIXCH_INFOBIT_REVERSEDVALUE}} };
...
static aucards_onemixerchan_s ess18xx_hwv_master_switch =
	{ AU_MIXCHANFUNCS_PACK(AU_MIXCHAN_MASTER, AU_MIXCHANFUNC_MUTE), 2, {{0x61, 1, 6, SUBMIXCH_INFOBIT_REVERSEDVALUE}, {0x63, 1, 6, SUBMIXCH_INFOBIT_REVERSEDVALUE}} };
...
static void ess_select_mixerinfo(unsigned int hardware)
{
	if(hardware & ESS_MIXERTYPE2) {
		if(hardware & ES18XX_HWV)
			ESS_sndcard_info.card_mixerchans = &ess18xx_hwv_mixer_info;
		else
			ESS_sndcard_info.card_mixerchans = &ess18xx_mixer_info;
	} else
		ESS_sndcard_info.card_mixerchans = &ess16xx_mixer_info;
}
