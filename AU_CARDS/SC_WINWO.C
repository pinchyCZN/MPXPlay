...
static void wavout_set_waveform(WAVEFORMATEX * wf, struct mpxplay_audioout_info_s *aui)
{
	...
	if(aui->freq_set) {
		...
		else if(freq > MPXP_WAVEMAPPER_MAX_FREQ)
...
static int WINWAVOUT_detect(struct mpxplay_audioout_info_s *aui)
{
	...
	card->devinfo = calloc(card->nb_devices + 1, sizeof(*card->devinfo));
	if(!card->devinfo)
		goto err_out_detect;

	if(aucards_select_devicenum && card->nb_devices) {
		unsigned int devnum = aucards_select_devicenum;
	...
	card->pcmout_buffer = (char *)malloc(card->pcmout_bufsize);
	if(!card->pcmout_buffer)
		goto err_out_detect;
	aui->card_DMABUFF = card->pcmout_buffer;

	card->nb_bufblock = MPXP_WAVEMAPPER_BUFHDR_BLOCKS;
	card->bufhdr = (WAVEHDR *) calloc(card->nb_bufblock, sizeof(WAVEHDR));
	if(!card->bufhdr)
		goto err_out_detect;
...
static void WINWAVOUT_card_info(struct mpxplay_audioout_info_s *aui)
{
	...
	if(card && card->devinfo) {
		for(i = 0; i < card->nb_devices + 1; i++) {
			...
			snprintf(sout, sizeof(sout), "%s dev%d: %s (chans:%d) %s", ((i == 0) ? "WIN :" : "     "), i, di->szPname, (int)di->wChannels, ((di == card->curr_dev) ? "(selected)" : ""));
...
one_sndcard_info WINWAVOUT_sndcard_info = {
	...
	&winwavout_mixerset			// card_mixerchans
};
